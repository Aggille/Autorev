...
var
  BoxNFeDevolucao: TBoxNFeDevolucao;

implementation

uses Empresas, Biblioteca, FDB, NFeComplementar;

{$R *.dfm}

procedure TBoxNFeDevolucao.BtnConfirmaClick(Sender: TObject);
begin
showmessage('em cosntrução');
{ with tblNofisa do
  begin
    Close;
...
procedure TBoxNFeDevolucao.GravarNFe;
begin
...
//     FieldByName('DESCRICAO_CODIGO_FISCAL').AsString := Valor.Value;
     FieldByName('Dados_Ad01').AsString := 'Nota fiscal de devolução da';
...
procedure TBoxNFeDevolucao.tblNofisaAfterCancel(DataSet: TDataSet);
begin
  Fdb1.IBTransaction.RollbackRetaining;
end;

procedure TBoxNFeDevolucao.tblNofisaAfterPost(DataSet: TDataSet);
begin
  FDb1.IBTransaction.CommitRetaining;
end;

procedure TBoxNFeDevolucao.FormClose(Sender: TObject;
...
procedure TBoxNFeDevolucao.FormKeyPress(Sender: TObject; var Key: Char);
begin
  if Key = #27 then
   begin
    Close;
...
procedure TBoxNFeDevolucao.FecharDs;
begin
  IBQuery2.Close;
  IBQuery1.Close;
