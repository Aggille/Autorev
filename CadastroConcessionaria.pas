...
type
  TBoxConcessionaria = class(TForm)
    Panel1: TPanel;
    ...
    procedure tblConcessionariaAfterCancel(DataSet: TDataSet);
    procedure tblConcessionariaAfterPost(DataSet: TDataSet);
  private
...
var
  BoxConcessionaria: TBoxConcessionaria;
  Retorno : String;
...
function TBoxConcessionaria.FieldsWrithe(DtSrc: TDataSource): Boolean;
var
  i : integer;
...
procedure TBoxConcessionaria.DoAfterClose;
begin
  DataSource1.Dataset.Close;
  Close;
end;

procedure TBoxConcessionaria.NovoExecute(Sender: TObject);
begin
  {: novo registro}
...
procedure TBoxConcessionaria.ConsultarExecute(Sender: TObject);
begin
...
procedure TBoxConcessionaria.EditarExecute(Sender: TObject);
begin
...
procedure TBoxConcessionaria.EditSequenciaClick(Sender: TObject);
begin
 BtnConsultar.Click;
end;

procedure TBoxConcessionaria.EditSequenciaExit(Sender: TObject);
var
   id : Integer;
begin
...
procedure TBoxConcessionaria.SalvarExecute(Sender: TObject);
begin
...
procedure TBoxConcessionaria.tblConcessionariaAfterCancel(DataSet: TDataSet);
begin
   IBTransaction1.RollbackRetaining;
end;

procedure TBoxConcessionaria.tblConcessionariaAfterPost(DataSet: TDataSet);
begin
    IBTransaction1.CommitRetaining;
end;

procedure TBoxConcessionaria.ExcluirExecute(Sender: TObject);
begin
...
procedure TBoxConcessionaria.ProcurarExecute(Sender: TObject);
begin
  ...
  if DataSource1.State in [dsEdit, dsInsert] then
   ...
   begin
     Retorno := Biblioteca.PesquisaGeral('Concessionária','Concessionaira',
      ['Id_Concessionaria','Codigo','Descricao'],
     ...
     begin
        ...
        Open;
        EditSequencia.Text := tblConcessionaria.FieldByName('Id_Concessionaria').Text;
     end;
     EnableDisableControls(False, False, True);
     BtnSalvar.Enabled := False;
     Salvar.Enabled := false;
...
procedure TBoxConcessionaria.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action :=caFree;
  BoxConcessionaria := nil;
end;

procedure TBoxConcessionaria.FormShow(Sender: TObject);
begin
  EnableDisableButtons(True, False);
  BtnConsultar.SetFocus;
end;

procedure TBoxConcessionaria.CancelarExecute(Sender: TObject);
begin
...
procedure TBoxConcessionaria.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TBoxConcessionaria.DBEditDescricaoKeyPress(Sender: TObject; var Key: Char);
begin
   {: se apertou ESC, fecha janela}
  if Key = #27 then
    begin
...
procedure TBoxConcessionaria.PermissoesForm;
begin
  {: implementado nos forms descendentes }
end;

procedure TBoxConcessionaria.Restricoes;
begin
  {: implementado nos forms descendentes }
end;

procedure TBoxConcessionaria.LimpaTela;
begin
  EditSequencia.Clear;
  DataSource1.Dataset.Close;
end;

procedure TBoxConcessionaria.EnableDisableControls(Value: Boolean; Value1: Boolean;
            Value2: Boolean);
var
  i : integer;
begin
  ...
  if (FDB1.IBDataSetLoginADM.AsString = 'T') Or
     (FDB1.IBDataSetLoginAcesso3.AsString = 'T') then
    begin
...
procedure TBoxConcessionaria.EnableDisableButtons(Value: Boolean; Value1: Boolean);
begin
