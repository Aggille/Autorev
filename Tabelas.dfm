object BoxTabelas: TBoxTabelas
  Left = 15
  Top = 85
  Caption = 'Tabelas'
  ClientHeight = 533
  ClientWidth = 1160
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesigned
  OnClose = FormClose
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object StatusBar1: TStatusBar
    Left = 0
    Top = 514
    Width = 1160
    Height = 19
    Panels = <>
  end
  object Panel1: TPanel
    Left = 0
    Top = 65
    Width = 1160
    Height = 449
    Align = alClient
    Caption = 'Panel1'
    TabOrder = 1
    object wwDBGrid1: TwwDBGrid
      AlignWithMargins = True
      Left = 7
      Top = 7
      Width = 1146
      Height = 226
      ControlType.Strings = (
        'NEGRITO;CheckBox;S;N')
      Selected.Strings = (
        'NEGRITO'#9'6'#9'Negrito:'#9'F'
        'ID_MODVEIC'#9'10'#9'Sequ'#234'ncia'#9'F'#9
        'MARCA'#9'10'#9'Marca:'#9'F'#9
        'MODELO'#9'20'#9'Modelo:'#9'F'#9
        'PRECO_VIP'#9'19'#9'Pre'#231'o VIP:'#9'F'
        'PRECO_NORMAL'#9'16'#9'Pre'#231'o normal:'#9'F'
        'PRECO_VENDA'#9'16'#9'Pre'#231'o exclusivo:'#9'F'
        'ANO_FABRICACAO'#9'8'#9'Ano fabr.:'#9'F'
        'ANO_MODELO'#9'10'#9'Ano modelo:'#9'F'
        'CILINDROS'#9'7'#9'Cilindros:'#9'F'#9
        'CILINDRADAS'#9'7'#9'Cc.:'#9'F'
        'HP'#9'6'#9'HP:'#9'F'
        'COMBUSTIVEL'#9'10'#9'Combust.:'#9'F'
        'RENAVAM'#9'9'#9'Renavam:'#9'F'
        'CST'#9'3'#9'CST'#9#9
        'VEICULO_NOVO'#9'13'#9'VEICULO_NOVO'#9#9)
      IniAttributes.Delimiter = ';;'
      IniAttributes.UnicodeIniFile = False
      TitleColor = clBtnFace
      FixedCols = 0
      ShowHorzScrollBar = True
      DataSource = DataSourceModelosVeiculos
      TabOrder = 0
      TitleAlignment = taCenter
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      TitleLines = 1
      TitleButtons = True
      UseTFields = False
      object wwDBGrid1IButton: TwwIButton
        Left = 0
        Top = 0
        Width = 13
        Height = 22
        AllowAllUp = True
      end
    end
    object wwDBGrid2: TwwDBGrid
      AlignWithMargins = True
      Left = 7
      Top = 292
      Width = 474
      Height = 141
      Selected.Strings = (
        'ID_VEICULOS_ACESSORIOS'#9'10'#9'Sequ'#234'ncia:'#9'F'
        'DESCRICAO_ACESSORIO'#9'20'#9'Descri'#231#227'o do acess'#243'rio:'#9'F'
        'VALOR'#9'19'#9'Valor:'#9'F'
        'MODELO'#9'20'#9'Modelo ve'#237'culo:'#9'F')
      IniAttributes.Delimiter = ';;'
      IniAttributes.UnicodeIniFile = False
      TitleColor = clBtnFace
      FixedCols = 0
      ShowHorzScrollBar = True
      DataSource = DataSourceAcessoriosVeiculos
      TabOrder = 1
      TitleAlignment = taCenter
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      TitleLines = 1
      TitleButtons = True
      UseTFields = False
      object wwIButton1: TwwIButton
        Left = 0
        Top = 0
        Width = 13
        Height = 22
        AllowAllUp = True
      end
    end
    object wwDBNavigator2: TwwDBNavigator
      Left = 496
      Top = 292
      Width = 350
      Height = 25
      DataSource = DataSourceAcessoriosVeiculos
      RepeatInterval.InitialDelay = 500
      RepeatInterval.Interval = 100
      object wwDBNavigator2First: TwwNavButton
        Left = 0
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Move to first record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2First'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 0
        Style = nbsFirst
      end
      object wwDBNavigator2PriorPage: TwwNavButton
        Left = 25
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Move backward 10 records'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2PriorPage'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 1
        Style = nbsPriorPage
      end
      object wwDBNavigator2Prior: TwwNavButton
        Left = 50
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Move to prior record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Prior'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 2
        Style = nbsPrior
      end
      object wwDBNavigator2Next: TwwNavButton
        Left = 75
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Move to next record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Next'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 3
        Style = nbsNext
      end
      object wwDBNavigator2NextPage: TwwNavButton
        Left = 100
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Move forward 10 records'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2NextPage'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 4
        Style = nbsNextPage
      end
      object wwDBNavigator2Last: TwwNavButton
        Left = 125
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Move to last record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Last'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 5
        Style = nbsLast
      end
      object wwDBNavigator2Insert: TwwNavButton
        Left = 150
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Insert new record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Insert'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 6
        Style = nbsInsert
      end
      object wwDBNavigator2Delete: TwwNavButton
        Left = 175
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Delete current record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Delete'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 7
        Style = nbsDelete
      end
      object wwDBNavigator2Edit: TwwNavButton
        Left = 200
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Edit current record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Edit'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 8
        Style = nbsEdit
      end
      object wwDBNavigator2Post: TwwNavButton
        Left = 225
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Post changes of current record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Post'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 9
        Style = nbsPost
      end
      object wwDBNavigator2Cancel: TwwNavButton
        Left = 250
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Cancel changes made to current record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Cancel'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 10
        Style = nbsCancel
      end
      object wwDBNavigator2Refresh: TwwNavButton
        Left = 275
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Refresh the contents of the dataset'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2Refresh'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 11
        Style = nbsRefresh
      end
      object wwDBNavigator2SaveBookmark: TwwNavButton
        Left = 300
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Bookmark current record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2SaveBookmark'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 12
        Style = nbsSaveBookmark
      end
      object wwDBNavigator2RestoreBookmark: TwwNavButton
        Left = 325
        Top = 0
        Width = 25
        Height = 25
        Hint = 'Go back to saved bookmark'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator2RestoreBookmark'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 13
        Style = nbsRestoreBookmark
      end
    end
    object capacete: TCurrencyEdit
      Left = 696
      Top = 368
      Width = 121
      Height = 21
      Margins.Left = 4
      Margins.Top = 1
      Enabled = False
      TabOrder = 3
      Visible = False
    end
  end
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 1160
    Height = 65
    ButtonHeight = 59
    Caption = 'ToolBar1'
    TabOrder = 0
    object wwDBNavigator1: TwwDBNavigator
      Left = 0
      Top = 0
      Width = 665
      Height = 59
      AutosizeStyle = asNone
      DataSource = DataSourceModelosVeiculos
      Flat = False
      ImageList = ImageList1
      RepeatInterval.InitialDelay = 500
      RepeatInterval.Interval = 100
      Align = alClient
      object wwDBNavigator1First: TwwNavButton
        Left = 0
        Top = 0
        Width = 86
        Height = 50
        Hint = 'Move to first record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator1First'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 0
        Style = nbsFirst
      end
      object wwDBNavigator1Last: TwwNavButton
        Left = 86
        Top = 0
        Width = 86
        Height = 50
        Hint = 'Move to last record'
        ImageIndex = -1
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator1Last'
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 1
        Style = nbsLast
      end
      object wwDBNavigator1Insert: TwwNavButton
        Left = 172
        Top = 0
        Width = 86
        Height = 50
        Hint = 'Cria um novo registro'
        ImageIndex = 2
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator1Insert'
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 2
        Style = nbsInsert
      end
      object wwDBNavigator1Edit: TwwNavButton
        Left = 258
        Top = 0
        Width = 86
        Height = 50
        Hint = 'Edit current record'
        ImageIndex = 6
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator1Edit'
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 3
        Style = nbsEdit
      end
      object wwDBNavigator1Post: TwwNavButton
        Left = 344
        Top = 0
        Width = 85
        Height = 50
        Hint = 'Post changes of current record'
        ImageIndex = 4
        NumGlyphs = 2
        Spacing = 4
        Transparent = False
        Caption = 'wwDBNavigator1Post'
        Enabled = False
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        Index = 4
        Style = nbsPost
      end
      object wwDBNavigator1Button: TwwNavButton
        Left = 429
        Top = 0
        Width = 84
        Height = 50
        Hint = 'Tabela Yamavale'
        ImageIndex = 10
        NumGlyphs = 1
        Spacing = 4
        Transparent = False
        Caption = 'Tabela Yamavale'
        DisabledTextColors.ShadeColor = clGray
        DisabledTextColors.HighlightColor = clBtnHighlight
        OnClick = wwDBNavigator1ButtonClick
        Index = 5
        Style = nbsCustom
      end
    end
  end
  object tblModelosVeiculos: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblModelosVeiculosAfterCancel
    AfterEdit = tblModelosVeiculosAfterPost
    AfterPost = tblModelosVeiculosAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from MODVEIC'
      'where'
      '  ID_MODVEIC = :OLD_ID_MODVEIC')
    InsertSQL.Strings = (
      'insert into MODVEIC'
      
        '  (ID_MODVEIC, MARCA, MODELO, CILINDROS, CILINDRADAS, HP, COMBUS' +
        'TIVEL, '
      
        '   RENAVAM, CLASSIF_FISCAL, PPS, PPS_ABRACY, PRECO_VENDA, CST, V' +
        'EICULO_NOVO, '
      
        '   ANO_FABRICACAO, ANO_MODELO, PRECO_VIP, PRECO_NORMAL, PRECO_EX' +
        'CLUSIVO2, '
      '   PRECO_NORMAL2, PRECO_VIP2, NEGRITO)'
      'values'
      
        '  (:ID_MODVEIC, :MARCA, :MODELO, :CILINDROS, :CILINDRADAS, :HP, ' +
        ':COMBUSTIVEL, '
      
        '   :RENAVAM, :CLASSIF_FISCAL, :PPS, :PPS_ABRACY, :PRECO_VENDA, :' +
        'CST, :VEICULO_NOVO, '
      
        '   :ANO_FABRICACAO, :ANO_MODELO, :PRECO_VIP, :PRECO_NORMAL, :PRE' +
        'CO_EXCLUSIVO2, '
      '   :PRECO_NORMAL2, :PRECO_VIP2, :NEGRITO)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_MODVEIC,'
      '  MARCA,'
      '  MODELO,'
      '  CILINDROS,'
      '  CILINDRADAS,'
      '  HP,'
      '  COMBUSTIVEL,'
      '  RENAVAM,'
      '  CLASSIF_FISCAL,'
      '  PPS,'
      '  PPS_ABRACY,'
      '  PRECO_VENDA,'
      '  CST,'
      '  VEICULO_NOVO,'
      '  ANO_FABRICACAO,'
      '  ANO_MODELO,'
      '  PRECO_VIP,'
      '  PRECO_NORMAL,'
      '  PRECO_EXCLUSIVO2,'
      '  PRECO_NORMAL2,'
      '  PRECO_VIP2,'
      '  NEGRITO'
      'from MODVEIC '
      'where'
      '  ID_MODVEIC = :ID_MODVEIC')
    SelectSQL.Strings = (
      'select modveic.*, veiculos_acessorios.*'
      ''
      'from MODVEIC'
      ''
      'inner join veiculos_acessorios on (modveic.modelo ='
      'veiculos_acessorios.modelo)'
      'where modveic.Veiculo_novo = '#39'T'#39
      
        'ORDER BY modveic.cilindradas, modveic.MODELO, modveic.ANO_FABRIC' +
        'ACAO, '
      'modveic.ANO_MODELO')
    ModifySQL.Strings = (
      'update MODVEIC'
      'set'
      '  ID_MODVEIC = :ID_MODVEIC,'
      '  MARCA = :MARCA,'
      '  MODELO = :MODELO,'
      '  CILINDROS = :CILINDROS,'
      '  CILINDRADAS = :CILINDRADAS,'
      '  HP = :HP,'
      '  COMBUSTIVEL = :COMBUSTIVEL,'
      '  RENAVAM = :RENAVAM,'
      '  CLASSIF_FISCAL = :CLASSIF_FISCAL,'
      '  PPS = :PPS,'
      '  PPS_ABRACY = :PPS_ABRACY,'
      '  PRECO_VENDA = :PRECO_VENDA,'
      '  CST = :CST,'
      '  VEICULO_NOVO = :VEICULO_NOVO,'
      '  ANO_FABRICACAO = :ANO_FABRICACAO,'
      '  ANO_MODELO = :ANO_MODELO,'
      '  PRECO_VIP = :PRECO_VIP,'
      '  PRECO_NORMAL = :PRECO_NORMAL,'
      '  PRECO_EXCLUSIVO2 = :PRECO_EXCLUSIVO2,'
      '  PRECO_NORMAL2 = :PRECO_NORMAL2,'
      '  PRECO_VIP2 = :PRECO_VIP2,'
      '  NEGRITO = :NEGRITO'
      'where'
      '  ID_MODVEIC = :OLD_ID_MODVEIC')
    ParamCheck = True
    UniDirectional = False
    Active = True
    Left = 336
    Top = 48
    object tblModelosVeiculosID_MODVEIC: TIntegerField
      DisplayLabel = 'Sequ'#234'ncia'
      DisplayWidth = 10
      FieldName = 'ID_MODVEIC'
      Origin = '"MODVEIC"."ID_MODVEIC"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object tblModelosVeiculosMARCA: TIBStringField
      DisplayLabel = 'Marca:'
      DisplayWidth = 10
      FieldName = 'MARCA'
      Origin = '"MODVEIC"."MARCA"'
      Size = 10
    end
    object tblModelosVeiculosMODELO: TIBStringField
      DisplayLabel = 'Modelo:'
      DisplayWidth = 20
      FieldName = 'MODELO'
      Origin = '"MODVEIC"."MODELO"'
    end
    object tblModelosVeiculosCILINDROS: TIntegerField
      DisplayLabel = 'Cilindros:'
      DisplayWidth = 10
      FieldName = 'CILINDROS'
      Origin = '"MODVEIC"."CILINDROS"'
    end
    object tblModelosVeiculosCILINDRADAS: TIntegerField
      DisplayLabel = 'Cilindradas:'
      DisplayWidth = 10
      FieldName = 'CILINDRADAS'
      Origin = '"MODVEIC"."CILINDRADAS"'
    end
    object tblModelosVeiculosCOMBUSTIVEL: TIBStringField
      DisplayLabel = 'Combust'#237'vel:'
      DisplayWidth = 10
      FieldName = 'COMBUSTIVEL'
      Origin = '"MODVEIC"."COMBUSTIVEL"'
      Size = 10
    end
    object tblModelosVeiculosRENAVAM: TIBStringField
      DisplayWidth = 15
      FieldName = 'RENAVAM'
      Origin = '"MODVEIC"."RENAVAM"'
      Size = 15
    end
    object tblModelosVeiculosCLASSIF_FISCAL: TIBStringField
      DisplayWidth = 10
      FieldName = 'CLASSIF_FISCAL'
      Origin = '"MODVEIC"."CLASSIF_FISCAL"'
      FixedChar = True
      Size = 10
    end
    object tblModelosVeiculosPPS: TIBBCDField
      DisplayWidth = 19
      FieldName = 'PPS'
      Origin = '"MODVEIC"."PPS"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosPPS_ABRACY: TIBBCDField
      DisplayWidth = 19
      FieldName = 'PPS_ABRACY'
      Origin = '"MODVEIC"."PPS_ABRACY"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosPRECO_VENDA: TIBBCDField
      DisplayWidth = 19
      FieldName = 'PRECO_VENDA'
      Origin = '"MODVEIC"."PRECO_VENDA"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosCST: TIBStringField
      DisplayWidth = 3
      FieldName = 'CST'
      Origin = '"MODVEIC"."CST"'
      Size = 3
    end
    object tblModelosVeiculosVEICULO_NOVO: TIBStringField
      DisplayWidth = 1
      FieldName = 'VEICULO_NOVO'
      Origin = '"MODVEIC"."VEICULO_NOVO"'
      Size = 1
    end
    object tblModelosVeiculosHP: TIBStringField
      FieldName = 'HP'
      Origin = '"MODVEIC"."HP"'
      Size = 6
    end
    object tblModelosVeiculosANO_FABRICACAO: TIBStringField
      FieldName = 'ANO_FABRICACAO'
      Origin = '"MODVEIC"."ANO_FABRICACAO"'
      Size = 4
    end
    object tblModelosVeiculosANO_MODELO: TIBStringField
      FieldName = 'ANO_MODELO'
      Origin = '"MODVEIC"."ANO_MODELO"'
      Size = 4
    end
    object tblModelosVeiculosPRECO_VIP: TIBBCDField
      FieldName = 'PRECO_VIP'
      Origin = '"MODVEIC"."PRECO_VIP"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosPRECO_NORMAL: TIBBCDField
      FieldName = 'PRECO_NORMAL'
      Origin = '"MODVEIC"."PRECO_NORMAL"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosPRECO_EXCLUSIVO2: TIBBCDField
      FieldName = 'PRECO_EXCLUSIVO2'
      Origin = '"MODVEIC"."PRECO_EXCLUSIVO2"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosPRECO_NORMAL2: TIBBCDField
      FieldName = 'PRECO_NORMAL2'
      Origin = '"MODVEIC"."PRECO_NORMAL2"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosPRECO_VIP2: TIBBCDField
      FieldName = 'PRECO_VIP2'
      Origin = '"MODVEIC"."PRECO_VIP2"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosID_VEICULOS_ACESSORIOS: TIntegerField
      FieldName = 'ID_VEICULOS_ACESSORIOS'
      Origin = '"VEICULOS_ACESSORIOS"."ID_VEICULOS_ACESSORIOS"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object tblModelosVeiculosDESCRICAO_ACESSORIO: TIBStringField
      FieldName = 'DESCRICAO_ACESSORIO'
      Origin = '"VEICULOS_ACESSORIOS"."DESCRICAO_ACESSORIO"'
    end
    object tblModelosVeiculosVALOR: TIBBCDField
      FieldName = 'VALOR'
      Origin = '"VEICULOS_ACESSORIOS"."VALOR"'
      Precision = 18
      Size = 2
    end
    object tblModelosVeiculosMODELO1: TIBStringField
      FieldName = 'MODELO1'
      Origin = '"VEICULOS_ACESSORIOS"."MODELO"'
    end
    object tblModelosVeiculosNEGRITO: TIBStringField
      FieldName = 'NEGRITO'
      Origin = '"MODVEIC"."NEGRITO"'
      Size = 1
    end
  end
  object DataSourceModelosVeiculos: TDataSource
    AutoEdit = False
    DataSet = tblModelosVeiculos
    Left = 424
    Top = 50
  end
  object ImageList1: TImageList
    Left = 304
    Top = 48
    Bitmap = {
      
      000000000000}
  end
  object tblRelModeloVeiculos: TfrxDBDataset
    UserName = 'tblRelModeloVeiculos'
    CloseDataSource = False
    FieldAliases.Strings = (
      'ID_MODVEIC=ID_MODVEIC'
      'MARCA=MARCA'
      'MODELO=MODELO'
      'CILINDROS=CILINDROS'
      'CILINDRADAS=CILINDRADAS'
      'COMBUSTIVEL=COMBUSTIVEL'
      'RENAVAM=RENAVAM'
      'CLASSIF_FISCAL=CLASSIF_FISCAL'
      'PPS=PPS'
      'PPS_ABRACY=PPS_ABRACY'
      'PRECO_VENDA=PRECO_VENDA'
      'CST=CST'
      'VEICULO_NOVO=VEICULO_NOVO'
      'HP=HP'
      'ANO_FABRICACAO=ANO_FABRICACAO'
      'ANO_MODELO=ANO_MODELO'
      'PRECO_VIP=PRECO_VIP'
      'PRECO_NORMAL=PRECO_NORMAL'
      'PRECO_EXCLUSIVO2=PRECO_EXCLUSIVO2'
      'PRECO_NORMAL2=PRECO_NORMAL2'
      'PRECO_VIP2=PRECO_VIP2'
      'ID_VEICULOS_ACESSORIOS=ID_VEICULOS_ACESSORIOS'
      'DESCRICAO_ACESSORIO=DESCRICAO_ACESSORIO'
      'VALOR=VALOR'
      'MODELO1=MODELO1'
      'NEGRITO=NEGRITO')
    DataSet = tblModelosVeiculos
    BCDToCurrency = False
    Left = 648
    Top = 128
  end
  object frxReport1: TfrxReport
    Version = '5.4.6'
    DotMatrixReport = False
    IniFile = '\Software\Fast Reports'
    PreviewOptions.Buttons = [pbPrint, pbLoad, pbSave, pbExport, pbZoom, pbFind, pbOutline, pbPageSetup, pbTools, pbEdit, pbNavigator, pbExportQuick]
    PreviewOptions.Zoom = 1.000000000000000000
    PrintOptions.Printer = 'Padr'#227'o'
    PrintOptions.PrintOnSheet = 0
    ReportOptions.CreateDate = 39991.584807546300000000
    ReportOptions.LastChange = 43011.398678298600000000
    ScriptLanguage = 'PascalScript'
    ScriptText.Strings = (
      '{var saldo : Real;'
      'procedure MasterData1OnBeforePrint(Sender: TfrxComponent);'
      
        '{                  [FormatMaskText(,)]<tblRelCaixa."VALOR"><tblR' +
        'elCaixa."VALOR"><tblRelCaixa."VALOR"><tblRelCaixa."VALOR">      ' +
        '      '
      'procedure Memo2OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      'If <tblRelNofisa."Ent_Sai"> = '#39'S'#39' then'
      '  begin            '
      '   Memo2.Text := '#39'X'#39';'
      '   Memo3.Text := '#39#39';                                         '
      '  end          '
      ' Else'
      '  begin             '
      '    Memo2.text := '#39#39';'
      '    Memo3.Text := '#39'X'#39';'
      '  end          '
      'end;'
      ''
      '{procedure DetailData1OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      'with BoxRelatorioNotasFiscais.tblRelNofisaFaturas1 do'
      ' begin'
      '  First;'
      '  If recno = 1 then                                             '
      '    Fatura1.Text := <tblRelNofisaFaturas1."FATURA">'
      '  else'
      '    Fatura1.Text := '#39#39';            '
      ' end;'
      'end;}  '
      'procedure Picture2OnBeforePrint(Sender: TfrxComponent);'
      'begin'
      '  picture2.loadfromfile('#39'c:\autorev-sd\logo.jpg'#39')  '
      'end;'
      ''
      
        'procedure tblRelModeloVeiculosMODELOOnBeforePrint(Sender: TfrxCo' +
        'mponent);'
      'begin'
      'if <tblRelModeloVeiculos."NEGRITO"> = '#39'S'#39' then'
      ' begin                       '
      '  tblRelModeloVeiculosMODELO.Font.Name := '#39'Arial Black'#39';  '
      '  tblRelModeloVeiculosEXCLUSIVA.Font.Name := '#39'Arial Black'#39';   '
      '  tblRelModeloVeiculosNORMAL.Font.Name := '#39'Arial Black'#39';   '
      '  tblRelModeloVeiculosANO_MODELO.Font.Name := '#39'Arial Black'#39';    '
      
        '  tblRelModeloVeiculosANO_FABRICACAO.Font.Name := '#39'Arial Black'#39';' +
        '    '
      '  tblRelModeloVeiculosVIP.Font.Name := '#39'Arial Black'#39';     '
      '  tblRelModeloVeiculosEMPLACAMENTO.Font.Name := '#39'Arial Black'#39';'
      '//  tblRelModeloVeiculosMODELO.Font.Style := fsBold;      '
      ' end          '
      'else                                       '
      ' begin            '
      '  tblRelModeloVeiculosMODELO.Font.Name := '#39'Arial'#39';       '
      '  tblRelModeloVeiculosEXCLUSIVA.Font.Name := '#39'Arial'#39';   '
      '  tblRelModeloVeiculosNORMAL.Font.Name := '#39'Arial'#39';   '
      '  tblRelModeloVeiculosANO_MODELO.Font.Name := '#39'Arial'#39';    '
      '  tblRelModeloVeiculosANO_FABRICACAO.Font.Name := '#39'Arial'#39';    '
      '  tblRelModeloVeiculosVIP.Font.Name := '#39'Arial'#39';     '
      '  tblRelModeloVeiculosEMPLACAMENTO.Font.Name := '#39'Arial'#39';      '
      ' end          '
      'end;'
      ''
      'begin'
      '  '
      'end.')
    OnBeforePrint = frxReport1BeforePrint
    Left = 608
    Top = 128
    Datasets = <
      item
        DataSet = tblRelModeloVeiculos
        DataSetName = 'tblRelModeloVeiculos'
      end>
    Variables = <>
    Style = <>
    object Data: TfrxDataPage
      Height = 1000.000000000000000000
      Width = 1000.000000000000000000
    end
    object Page1: TfrxReportPage
      PaperWidth = 210.000000000000000000
      PaperHeight = 297.000000000000000000
      PaperSize = 9
      object ReportTitle1: TfrxReportTitle
        FillType = ftBrush
        Height = 83.149660000000000000
        Top = 18.897650000000000000
        Width = 793.701300000000000000
        object Picture1: TfrxPictureView
          Left = 600.945270000000000000
          Top = 11.338590000000000000
          Width = 154.960730000000000000
          Height = 34.015770000000000000
          Picture.Data = {
            
            E284231D909451452343FFD9}
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo1: TfrxMemoView
          Left = 600.945270000000000000
          Top = 45.354360000000000000
          Width = 154.960730000000000000
          Height = 15.118120000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'Concession'#225'ria Autorizada')
          ParentFont = False
        end
        object Memo2: TfrxMemoView
          Left = 382.512060000000000000
          Top = 60.252010000000000000
          Width = 192.756030000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -19
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          HAlign = haCenter
          Memo.UTF8W = (
            'TABELA VENDAS')
          ParentFont = False
        end
        object Picture2: TfrxPictureView
          Left = 22.677180000000000000
          Top = 11.338590000000000000
          Width = 336.378170000000000000
          Height = 71.811070000000000000
          OnBeforePrint = 'Picture2OnBeforePrint'
          Picture.Data = {
            
            8A28A9374ADA23FFD9}
          HightQuality = False
          Transparent = False
          TransparentColor = clWhite
        end
        object Memo4: TfrxMemoView
          Left = 582.047620000000000000
          Top = 60.472480000000000000
          Width = 109.606370000000000000
          Height = 18.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            'V'#225'lida a partir de:')
          ParentFont = False
        end
        object Memo9: TfrxMemoView
          Left = 691.653990000000000000
          Top = 60.472480000000000000
          Width = 79.370130000000000000
          Height = 18.897650000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[Date]')
          ParentFont = False
        end
      end
      object MasterData1: TfrxMasterData
        FillType = ftBrush
        Height = 22.897650000000000000
        Top = 211.653680000000000000
        Width = 793.701300000000000000
        DataSet = tblRelModeloVeiculos
        DataSetName = 'tblRelModeloVeiculos'
        RowCount = 0
        object tblRelModeloVeiculosMODELO: TfrxMemoView
          Left = 23.055125430000000000
          Top = 4.000000000000000000
          Width = 196.535560000000000000
          Height = 18.897650000000000000
          OnBeforePrint = 'tblRelModeloVeiculosMODELOOnBeforePrint'
          DataField = 'MODELO'
          DataSet = tblRelModeloVeiculos
          DataSetName = 'tblRelModeloVeiculos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[tblRelModeloVeiculos."MODELO"]')
          ParentFont = False
        end
        object tblRelModeloVeiculosEXCLUSIVA: TfrxMemoView
          Left = 563.149970000000100000
          Top = 3.779529999999994000
          Width = 120.944960000000000000
          Height = 18.897650000000000000
          DataSet = tblRelModeloVeiculos
          DataSetName = 'tblRelModeloVeiculos'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '[<tblRelModeloVeiculos."PRECO_VENDA">+<tblRelModeloVeiculos."VAL' +
              'OR">+<capacete>]')
          ParentFont = False
        end
        object tblRelModeloVeiculosNORMAL: TfrxMemoView
          Left = 438.425480000000000000
          Top = 3.779529999999994000
          Width = 124.724490000000000000
          Height = 18.897650000000000000
          DataSet = tblRelModeloVeiculos
          DataSetName = 'tblRelModeloVeiculos'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '[<tblRelModeloVeiculos."PRECO_NORMAL">+<tblRelModeloVeiculos."VA' +
              'LOR">+<capacete>]')
          ParentFont = False
        end
        object tblRelModeloVeiculosANO_MODELO: TfrxMemoView
          Left = 268.346630000000000000
          Top = 4.000000000000028000
          Width = 45.354360000000000000
          Height = 18.897650000000000000
          DataField = 'ANO_MODELO'
          DataSet = tblRelModeloVeiculos
          DataSetName = 'tblRelModeloVeiculos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[tblRelModeloVeiculos."ANO_MODELO"]')
          ParentFont = False
        end
        object tblRelModeloVeiculosANO_FABRICACAO: TfrxMemoView
          Left = 219.212740000000000000
          Top = 4.000000000000028000
          Width = 49.133890000000000000
          Height = 18.897650000000000000
          DataField = 'ANO_FABRICACAO'
          DataSet = tblRelModeloVeiculos
          DataSetName = 'tblRelModeloVeiculos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Memo.UTF8W = (
            '[tblRelModeloVeiculos."ANO_FABRICACAO"]')
          ParentFont = False
        end
        object tblRelModeloVeiculosVIP: TfrxMemoView
          Left = 313.700990000000000000
          Top = 3.779529999999994000
          Width = 124.724490000000000000
          Height = 18.897650000000000000
          DataSet = tblRelModeloVeiculos
          DataSetName = 'tblRelModeloVeiculos'
          DisplayFormat.FormatStr = '%2.2m'
          DisplayFormat.Kind = fkNumeric
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            
              '[<tblRelModeloVeiculos."PRECO_VIP">+<tblRelModeloVeiculos."VALOR' +
              '">+<capacete>]')
          ParentFont = False
        end
        object tblRelModeloVeiculosEMPLACAMENTO: TfrxMemoView
          Left = 684.094930000000000000
          Top = 3.779529999999994000
          Width = 90.708720000000000000
          Height = 18.897650000000000000
          DataSet = tblRelModeloVeiculos
          DataSetName = 'tblRelModeloVeiculos'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          HAlign = haRight
          Memo.UTF8W = (
            '[(<tblRelModeloVeiculos."VALOR">+<capacete>)]')
          ParentFont = False
          Formats = <
            item
              FormatStr = '%2.2m'
              Kind = fkNumeric
            end
            item
            end>
        end
      end
      object Header1: TfrxHeader
        FillType = ftBrush
        Height = 26.456712440000000000
        Top = 162.519790000000000000
        Width = 793.701300000000000000
        object Memo5: TfrxMemoView
          Left = 22.897650000000000000
          Width = 196.535560000000000000
          Height = 26.456710000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Fill.BackColor = 14211288
          Memo.UTF8W = (
            'Modelo:')
          ParentFont = False
        end
        object Memo6: TfrxMemoView
          Left = 219.212740000000000000
          Width = 49.133890000000000000
          Height = 26.456710000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Fill.BackColor = 14211288
          Memo.UTF8W = (
            'Ano Fabr.:')
          ParentFont = False
        end
        object Memo7: TfrxMemoView
          Left = 438.425480000000000000
          Width = 124.724490000000000000
          Height = 26.456710000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Fill.BackColor = 14211288
          HAlign = haCenter
          Memo.UTF8W = (
            'Tabela normal:')
          ParentFont = False
        end
        object Memo8: TfrxMemoView
          Left = 563.149970000000100000
          Width = 120.944960000000000000
          Height = 26.456710000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Fill.BackColor = 14211288
          HAlign = haCenter
          Memo.UTF8W = (
            'Tabela exclusiva:')
          ParentFont = False
        end
        object Memo3: TfrxMemoView
          Left = 268.346630000000000000
          Top = 0.000002440000002935
          Width = 45.354360000000000000
          Height = 26.456710000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -9
          Font.Name = 'Arial'
          Font.Style = []
          Fill.BackColor = 14211288
          Memo.UTF8W = (
            'Ano Mod.:')
          ParentFont = False
        end
        object Memo11: TfrxMemoView
          Left = 313.700990000000000000
          Top = 0.000002440000002935
          Width = 124.724490000000000000
          Height = 26.456710000000000000
          DisplayFormat.DecimalSeparator = ','
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = []
          Fill.BackColor = 14211288
          HAlign = haCenter
          Memo.UTF8W = (
            'Tabela VIP:')
          ParentFont = False
        end
        object Memo12: TfrxMemoView
          Left = 684.094930000000000000
          Top = 0.000002440000002935
          Width = 94.488250000000000000
          Height = 26.456710000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -11
          Font.Name = 'Arial'
          Font.Style = []
          Fill.BackColor = 14211288
          HAlign = haCenter
          Memo.UTF8W = (
            'Emplacamento+ Capacete:')
          ParentFont = False
        end
      end
      object Footer1: TfrxFooter
        FillType = ftBrush
        Height = 260.787570000000000000
        Top = 257.008040000000000000
        Width = 793.701300000000000000
        object SysMemo1: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 11.338590000000010000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            
              'REGRAS PARA PARCELAMENTO DIRETO COM CART'#195'O - N'#195'O ACEITAMOS CHEQU' +
              'ES')
        end
        object Line1: TfrxLineView
          Left = 11.338590000000000000
          Top = 7.559059999999988000
          Width = 774.803650000000000000
          Color = clBlack
          ArrowWidth = 1
          Frame.Typ = [ftTop]
          Frame.TopLine.Width = 5.000000000000000000
          Frame.BottomLine.Width = 5.000000000000000000
        end
        object SysMemo2: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 30.236240000000010000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            
              'V'#193'LIDO PARA VALORES PEQUENOS - FACILITA'#199#195'O DE ENTRADA - DEMAIS V' +
              'ALORES SOB CONSULTA')
        end
        object SysMemo3: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 49.133890000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            
              'At'#233' 3 parcelas - Fator 1,06               4 parcelas - Fator 1,0' +
              '8             5 parcelas - Fator 1,10')
        end
        object SysMemo4: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 68.031540000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            
              '6 parcelas - Fator 1,12                     7 parcelas - Fator 1' +
              ',14             8 parcelas - Fator 1,16')
        end
        object SysMemo5: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 86.929190000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            
              '9 parcelas - Fator 1,18                   10 parcelas - Fator 1,' +
              '20           11 parcelas - Fator 1,22                 12 parcela' +
              's - Fator 1,24')
        end
        object SysMemo7: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 105.826840000000000000
          Width = 411.968770000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial'
          Font.Style = [fsBold]
          Memo.UTF8W = (
            'N'#195'O ACEITAMOS MAIS CHEQUES - CONV'#202'NIO CANCELADO')
          ParentFont = False
        end
        object SysMemo8: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 143.622140000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            'FRETES DE ENTREGA DE MOTOCICLETAS:')
        end
        object SysMemo9: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 162.519790000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            'Entregas em POA e Grande POA       R$ 100,00')
        end
        object SysMemo10: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 181.417440000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            'Entregas no litoral e serra                   R$ 400,00')
        end
        object SysMemo11: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 200.315090000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            'Demais localidades sob consulta')
        end
        object SysMemo12: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 124.724490000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial Black'
          Font.Style = []
          Memo.UTF8W = (
            'TRANSFER'#202'NCIAS DE USADOS            R$ 650,00')
          ParentFont = False
        end
        object SysMemo13: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 219.212740000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clBlack
          Font.Height = -13
          Font.Name = 'Arial Black'
          Font.Style = []
          Memo.UTF8W = (
            
              'Cons'#243'rcios de outras concession'#225'rias YAMAHA deve ser vendido na ' +
              'tabela VIP')
          ParentFont = False
        end
        object SysMemo16: TfrxSysMemoView
          Left = 22.677180000000000000
          Top = 238.110390000000000000
          Width = 740.787880000000000000
          Height = 18.897650000000000000
          Memo.UTF8W = (
            'Cart'#227'o BNDES utilizar tabela NORMAL OU VIP')
        end
      end
    end
  end
  object tblAcessoriosVeiculos: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from Veiculos_Acessorios'
      'where'
      '  ID_VEICULOS_ACESSORIOS = :OLD_ID_VEICULOS_ACESSORIOS')
    InsertSQL.Strings = (
      'insert into Veiculos_Acessorios'
      '  (DESCRICAO_ACESSORIO, ID_VEICULOS_ACESSORIOS, MODELO, VALOR)'
      'values'
      
        '  (:DESCRICAO_ACESSORIO, :ID_VEICULOS_ACESSORIOS, :MODELO, :VALO' +
        'R)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_VEICULOS_ACESSORIOS,'
      '  DESCRICAO_ACESSORIO,'
      '  VALOR,'
      '  MODELO'
      'from Veiculos_Acessorios '
      'where'
      '  ID_VEICULOS_ACESSORIOS = :ID_VEICULOS_ACESSORIOS')
    SelectSQL.Strings = (
      'select * from Veiculos_Acessorios'
      'WHERE DESCRICAO_ACESSORIO = '#39'EMPLACAMENTO'#39
      'ORDER BY MODELO'
      ''
      '')
    ModifySQL.Strings = (
      'update Veiculos_Acessorios'
      'set'
      '  DESCRICAO_ACESSORIO = :DESCRICAO_ACESSORIO,'
      '  ID_VEICULOS_ACESSORIOS = :ID_VEICULOS_ACESSORIOS,'
      '  MODELO = :MODELO,'
      '  VALOR = :VALOR'
      'where'
      '  ID_VEICULOS_ACESSORIOS = :OLD_ID_VEICULOS_ACESSORIOS')
    ParamCheck = True
    UniDirectional = False
    Left = 432
    Top = 408
    object tblAcessoriosVeiculosID_VEICULOS_ACESSORIOS: TIntegerField
      FieldName = 'ID_VEICULOS_ACESSORIOS'
      Origin = '"VEICULOS_ACESSORIOS"."ID_VEICULOS_ACESSORIOS"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object tblAcessoriosVeiculosDESCRICAO_ACESSORIO: TIBStringField
      FieldName = 'DESCRICAO_ACESSORIO'
      Origin = '"VEICULOS_ACESSORIOS"."DESCRICAO_ACESSORIO"'
    end
    object tblAcessoriosVeiculosVALOR: TIBBCDField
      FieldName = 'VALOR'
      Origin = '"VEICULOS_ACESSORIOS"."VALOR"'
      Precision = 18
      Size = 2
    end
    object tblAcessoriosVeiculosMODELO: TIBStringField
      FieldName = 'MODELO'
      Origin = '"VEICULOS_ACESSORIOS"."MODELO"'
    end
  end
  object DataSourceAcessoriosVeiculos: TDataSource
    AutoEdit = False
    DataSet = tblAcessoriosVeiculos
    Left = 400
    Top = 410
  end
end
