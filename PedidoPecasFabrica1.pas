...
type
  TBoxPedidoPecasFabrica = class(TForm)
    ...
    procedure DataPedidoExit(Sender: TObject);
...
begin
   AssignFile(ArquivoTexto,'C:\PEC_PED.txt'); {Associa o arquivo a uma variável do tipo textfile}
   ReWrite(ArquivoTexto); {Recria o arquivo texto}
   linha := '0'+ BoxEmpresas.CodigoConcessionaria + BoxEmpresas.CNPJ + 'SIACY';
   ...
   with tblPedidoDados do
    begin
      ...
      while not eof do
        begin
          linha := '21';
          linha := linha + StrZero(FieldByName('id_Pedido_pecas_fabrica_dados').AsInteger,10,0);
          linha := linha + FieldByName('Codigo').AsString;
...
procedure TBoxPedidoPecasFabrica.DataPedidoExit(Sender: TObject);
begin
  tblPedidos.Post;
  tblPedidos.Edit;
  with tblPedidoDados do
