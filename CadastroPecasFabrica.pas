...
type
  TBoxPecasFabrica = class(TForm)
    ...
    procedure tblPecasFabricaAfterCancel(DataSet: TDataSet);
    procedure tblPecasFabricaAfterPost(DataSet: TDataSet);
  private
...
var
  BoxPecasFabrica: TBoxPecasFabrica;
  Retorno : String;
...
function TBoxPecasFabrica.FieldsWrithe(DtSrc: TDataSource): Boolean;
var
  i : integer;
...
procedure TBoxPecasFabrica.FormShow(Sender: TObject);
begin
  EnableDisableButtons(True, False);
  BtnConsultar.SetFocus;
end;

procedure TBoxPecasFabrica.DoAfterClose;
begin
  DataSource1.Dataset.Close;
  Close;
end;

procedure TBoxPecasFabrica.NovoExecute(Sender: TObject);
begin
  {: novo registro}
...
procedure TBoxPecasFabrica.ConsultarExecute(Sender: TObject);
begin
...
procedure TBoxPecasFabrica.EditarExecute(Sender: TObject);
begin
...
procedure TBoxPecasFabrica.EditSequenciaExit(Sender: TObject);
var
   id : Integer;
begin
...
procedure TBoxPecasFabrica.SalvarExecute(Sender: TObject);
begin
...
procedure TBoxPecasFabrica.tblPecasFabricaAfterCancel(DataSet: TDataSet);
begin
   FDB1.IBTransaction.RollbackRetaining;
end;

procedure TBoxPecasFabrica.tblPecasFabricaAfterPost(DataSet: TDataSet);
begin
    FDb1.IBTransaction.CommitRetaining;
end;

procedure TBoxPecasFabrica.ExcluirExecute(Sender: TObject);
begin
...
procedure TBoxPecasFabrica.ProcurarExecute(Sender: TObject);
begin
  ...
  if DataSource1.State in [dsEdit, dsInsert] then
   ...
   begin
     Retorno := Biblioteca.PesquisaGeral('peças da fábrica','PECYAM',
      ['Codigo','Id_Pecyam','Descricao'],
      ['Código:','Sequência:','Descrição:',''],
     ...
     begin
        ...
        Open;
        EditSequencia.Text := tblPecasFabrica.FieldByName('Id_Pecyam').Text;
     end;
     EnableDisableControls(False, False, True);
     BtnSalvar.Enabled := False;
     Salvar.Enabled := False;
...
procedure TBoxPecasFabrica.CancelarExecute(Sender: TObject);
begin
...
procedure TBoxPecasFabrica.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action :=caFree;
  BoxPecasFabrica := nil;
end;

procedure TBoxPecasFabrica.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TBoxPecasFabrica.DBEditDescricaoKeyPress(Sender: TObject; var Key: Char);
begin
   {: se apertou ESC, fecha janela}
  if Key = #27 then
    begin
...
procedure TBoxPecasFabrica.PermissoesForm;
begin
  {: implementado nos forms descendentes }
end;

procedure TBoxPecasFabrica.Restricoes;
begin
  {: implementado nos forms descendentes }
end;

procedure TBoxPecasFabrica.LimpaTela;
begin
  EditSequencia.Clear;
  DataSource1.Dataset.Close;
end;

procedure TBoxPecasFabrica.EnableDisableControls(Value: Boolean; Value1: Boolean;
            Value2: Boolean);
var
  i : integer;
begin
...
procedure TBoxPecasFabrica.EnableDisableButtons(Value: Boolean; Value1: Boolean);
begin
