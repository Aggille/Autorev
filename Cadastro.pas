...
uses
   ...
   BIBLIOTECA, PESQUISACFOP, FDB;

{$R *.dfm}

function TBox.FieldsWrithe(DtSrc: TDataSource): Boolean;
var
  i : integer;
...
procedure TBox.DoAfterClose;
begin
  DataSource1.Dataset.Close;
  Close;
end;

procedure TBox.NovoExecute(Sender: TObject);
begin
  {: novo registro}
...
procedure TBox.ConsultarExecute(Sender: TObject);
begin
...
procedure TBox.EditarExecute(Sender: TObject);
begin
...
procedure TBox.EditSequenciaClick(Sender: TObject);
begin
 BtnConsultar.Click;
end;

procedure TBox.EditSequenciaExit(Sender: TObject);
var
   id : Integer;
begin
...
procedure TBox.SalvarExecute(Sender: TObject);
begin
...
procedure TBox.tblCfopAfterCancel(DataSet: TDataSet);
begin
   IBTransaction1.RollbackRetaining;
end;

procedure TBox.tblCfopAfterPost(DataSet: TDataSet);
begin
    IBTransaction1.CommitRetaining;
end;

procedure TBox.ExcluirExecute(Sender: TObject);
begin
...
procedure TBox.ProcurarExecute(Sender: TObject);
begin
  ...
  if DataSource1.State in [dsEdit, dsInsert] then
   ...
   begin
    if not Assigned(BoxPesquisarCFOP) then
      BoxPesquisarCFOP := TBoxPesquisarCFOP.Create(Self);
    BoxPesquisarCFOP.Show;
   end;
end;

procedure TBox.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action :=caFree;
  BoxCfop := nil;
end;
...
procedure TBox.CancelarExecute(Sender: TObject);
begin
...
procedure TBox.FormKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TBox.DBEditDescricaoKeyPress(Sender: TObject; var Key: Char);
begin
   {: se apertou ESC, fecha janela}
  if Key = #27 then
    begin
...
procedure TBox.PermissoesForm;
begin
  {: implementado nos forms descendentes }
end;

procedure TBox.Restricoes;
begin
  {: implementado nos forms descendentes }
end;

procedure TBox.LimpaTela;
begin
  EditSequencia.Clear;
  DataSource1.Dataset.Close;
end;

procedure TBox.EnableDisableControls(Value: Boolean; Value1: Boolean;
            Value2: Boolean);
var
  i : integer;
begin
...
procedure TBox.EnableDisableButtons(Value: Boolean; Value1: Boolean);
begin
