object BoxPecas: TBoxPecas
  Left = 0
  Top = 100
  Caption = 'Cadastro de Pe'#231'as'
  ClientHeight = 414
  ClientWidth = 910
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesigned
  OnClose = FormClose
  OnCloseQuery = FormCloseQuery
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 910
    Height = 37
    ButtonHeight = 21
    Caption = 'ToolBar1'
    TabOrder = 0
    object BtnProcurar: TBitBtn
      Left = 0
      Top = 0
      Width = 81
      Height = 21
      Hint = 'Procurar registro'
      Action = Procurar
      Align = alClient
      Caption = 'F4-&Procurar'
      Glyph.Data = {
        
        00FFFF00FFA2A2A2A3A3A39F9F9FFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnKeyPress = FormKeyPress
    end
    object BtnConsultar: TBitBtn
      Left = 81
      Top = 0
      Width = 85
      Height = 21
      Hint = 'Consultar registro'
      Action = Consultar
      Caption = 'F5-C&onsultar'
      Glyph.Data = {
        
        B1B1B1B1B1B1B1B1B1B1B1999999FF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnKeyPress = FormKeyPress
    end
    object BtnNovo: TBitBtn
      Left = 166
      Top = 0
      Width = 93
      Height = 21
      Hint = 'Registro novo'
      Action = Novo
      Caption = 'F6-&Novo'
      Glyph.Data = {
        
        B4B4B4B4B4B4B4B4B4B4B4787878FF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnKeyPress = FormKeyPress
    end
    object BtnEditar: TBitBtn
      Left = 259
      Top = 0
      Width = 94
      Height = 21
      Hint = 'Editar registro'
      Action = Editar
      Align = alClient
      Caption = 'F7-&Editar'
      Glyph.Data = {
        
        6666FF00FF666666FF00FF666666848484848484FF00FFFF00FF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnKeyPress = FormKeyPress
    end
    object BtnExcluir: TBitBtn
      Left = 353
      Top = 0
      Width = 84
      Height = 21
      Hint = 'Excluir registro'
      Action = Excluir
      Caption = 'F8-E&xcluir'
      Enabled = False
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnKeyPress = FormKeyPress
    end
    object BtnCancelar: TBitBtn
      Left = 437
      Top = 0
      Width = 85
      Height = 21
      Hint = 'Cancelar registro'
      Action = Cancelar
      Caption = 'F9-&Cancelar'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      TabOrder = 5
      OnKeyPress = FormKeyPress
    end
    object BtnSalvar: TBitBtn
      Left = 522
      Top = 0
      Width = 81
      Height = 21
      Hint = 'Salvar registro'
      Action = Salvar
      Caption = 'F10-&Salvar'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnKeyPress = FormKeyPress
    end
  end
  object PageControl: TPageControl
    Left = 0
    Top = 37
    Width = 910
    Height = 358
    ActivePage = TabSheetMovimentacao
    Align = alClient
    TabOrder = 1
    object TabSheetDadosGerais: TTabSheet
      Caption = 'Dados Gerais'
      object PrecoVendaLabel: TLabel
        Left = 6
        Top = 87
        Width = 79
        Height = 13
        Caption = 'Pre'#231'o de Venda:'
      end
      object UnidadeLabel: TLabel
        Left = 299
        Top = 87
        Width = 43
        Height = 13
        Caption = 'Unidade:'
      end
      object Label1: TLabel
        Left = 6
        Top = 6
        Width = 53
        Height = 13
        Caption = 'Sequ'#234'ncia:'
      end
      object Label2: TLabel
        Left = 6
        Top = 60
        Width = 50
        Height = 13
        Caption = 'Descri'#231#227'o:'
      end
      object Label3: TLabel
        Left = 6
        Top = 33
        Width = 37
        Height = 13
        Caption = 'C'#243'digo:'
      end
      object Label12: TLabel
        Left = 196
        Top = 87
        Width = 33
        Height = 13
        Caption = 'Grupo:'
      end
      object Label16: TLabel
        Left = 304
        Top = 11
        Width = 63
        Height = 13
        Caption = 'Qtde m'#237'nima:'
      end
      object Descricao: TJvDBMaskEdit
        Left = 89
        Top = 57
        Width = 309
        Height = 21
        Hint = 'Digite a descri'#231#227'o'
        CharCase = ecUpperCase
        DataField = 'DESCRICAO'
        DataSource = DataSourcePecas
        MaxLength = 60
        TabOrder = 5
        EditMask = 'cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc;1; '
        OnKeyPress = FormKeyPress
      end
      object PrecoVenda: TJvDBMaskEdit
        Left = 89
        Top = 84
        Width = 76
        Height = 21
        DataField = 'PRECO'
        DataSource = DataSourcePecas
        Enabled = False
        ReadOnly = True
        TabOrder = 6
        EditMask = ''
        OnKeyPress = FormKeyPress
      end
      object RadioGroup1: TRadioGroup
        Left = 212
        Top = 0
        Width = 86
        Height = 51
        Caption = 'Pe'#231'a:'
        Items.Strings = (
          'Original'
          'Similar')
        TabOrder = 0
        OnExit = RadioGroup1Exit
      end
      object EditSequencia: TMaskEdit
        Left = 89
        Top = 3
        Width = 44
        Height = 21
        Hint = 'Digite a sequ'#234'ncia'
        TabOrder = 2
        Text = ''
        OnExit = EditSequenciaExit
        OnKeyPress = FormKeyPress
      end
      object Grupo: TJvDBMaskEdit
        Left = 235
        Top = 84
        Width = 33
        Height = 21
        Hint = 'Digite o grupo'
        DataField = 'GRUPO'
        DataSource = DataSourcePecas
        MaxLength = 3
        TabOrder = 7
        EditMask = 'ccc;1; '
        OnKeyPress = FormKeyPress
      end
      object DBGrid2: TDBGrid
        Left = 3
        Top = 175
        Width = 580
        Height = 154
        DataSource = DataSourcePecasEstoque
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
        ReadOnly = True
        TabOrder = 11
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnExit = DBGrid3Exit
        Columns = <
          item
            Expanded = False
            FieldName = 'EMPRESA'
            Title.Alignment = taCenter
            Title.Caption = 'Concession'#225'ria:'
            Width = 327
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ESTOQUE'
            Title.Alignment = taCenter
            Title.Caption = 'Estoque:'
            Width = 53
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ESTOQOS'
            Title.Alignment = taCenter
            Title.Caption = 'Estoque em O.S.:'
            Width = 92
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'LOCAL'
            Title.Alignment = taCenter
            Title.Caption = 'Localiza'#231#227'o:'
            Width = 72
            Visible = True
          end>
      end
      object DBGrid3: TDBGrid
        Left = 3
        Top = 111
        Width = 576
        Height = 58
        DataSource = DataSourceEstoque
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
        ReadOnly = True
        TabOrder = 9
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnExit = DBGrid3Exit
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_PECAS_ESTOQUE'
            Title.Alignment = taCenter
            Title.Caption = 'Seq.Est.:'
            Width = 47
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ID_PECAS'
            Title.Alignment = taCenter
            Title.Caption = 'Seq.Cad.:'
            Width = 52
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ID_CONCESSIONARIA'
            Title.Alignment = taCenter
            Title.Caption = 'Concession'#225'ria:'
            Width = 239
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ESTOQUE'
            Title.Alignment = taCenter
            Title.Caption = 'Estoque:'
            Width = 50
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ESTOQOS'
            Title.Alignment = taCenter
            Title.Caption = 'Estoque em O.S.:'
            Width = 92
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'LOCAL'
            Title.Alignment = taCenter
            Title.Caption = 'Localiza'#231#227'o:'
            Width = 83
            Visible = True
          end>
      end
      object Unidade: TDBComboBox
        Left = 351
        Top = 84
        Width = 50
        Height = 21
        Hint = 'Selecione a unidade'
        DataField = 'UNIDADE'
        DataSource = DataSourcePecas
        Items.Strings = (
          'PR'
          'LT'
          'PC')
        TabOrder = 8
        OnExit = UnidadeExit
        OnKeyPress = FormKeyPress
      end
      object Codigo: TJvMaskEdit
        Left = 89
        Top = 30
        Width = 117
        Height = 21
        CharCase = ecUpperCase
        EditMask = 'cccccccccccc;1; '
        MaxLength = 12
        TabOrder = 3
        Text = '            '
        OnExit = CodigoExit
        OnKeyPress = FormKeyPress
      end
      object DBGrid4: TDBGrid
        Left = 589
        Top = 111
        Width = 315
        Height = 83
        DataSource = DataSourcePecasLocalizacao2
        Enabled = False
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 10
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnExit = DBGrid3Exit
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_PECAS'
            Title.Alignment = taCenter
            Title.Caption = 'Id Pe'#231'as:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'PRATELEIRA'
            Title.Alignment = taCenter
            Title.Caption = 'Prateleira:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ESCANINHO_GAVETA'
            Title.Alignment = taCenter
            Title.Caption = 'Escaninho:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CORREDOR'
            Title.Alignment = taCenter
            Title.Caption = 'Corredor:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ID_CONCESSIONARIA'
            Title.Alignment = taCenter
            Title.Caption = 'Concess.:'
            Width = 51
            Visible = True
          end>
      end
      object Panel1: TPanel
        Left = 585
        Top = 0
        Width = 315
        Height = 105
        TabOrder = 1
        object Label13: TLabel
          Left = 8
          Top = 84
          Width = 50
          Height = 13
          Caption = 'Prateleira:'
        end
        object Label15: TLabel
          Left = 8
          Top = 59
          Width = 47
          Height = 13
          Caption = 'Corredor:'
        end
        object EstoqueOSLabel: TLabel
          Left = 8
          Top = 33
          Width = 77
          Height = 13
          Caption = 'Estoque em OS:'
        end
        object EstoqueLabel: TLabel
          Left = 8
          Top = 7
          Width = 43
          Height = 13
          Caption = 'Estoque:'
        end
        object Label14: TLabel
          Left = 142
          Top = 84
          Width = 90
          Height = 13
          Caption = 'Escaninho/gaveta:'
        end
        object LocalizacaoLabel: TLabel
          Left = 141
          Top = 59
          Width = 58
          Height = 13
          Caption = 'Localiza'#231#227'o:'
          WordWrap = True
          IsControl = True
        end
        object JvDBMaskEditEstoque: TJvDBMaskEdit
          Left = 85
          Top = 4
          Width = 42
          Height = 21
          DataField = 'ESTOQUE'
          DataSource = DataSourceEstoque
          ReadOnly = True
          TabOrder = 0
          EditMask = ''
        end
        object JvDBMaskEditEstoqOS: TJvDBMaskEdit
          Left = 85
          Top = 30
          Width = 42
          Height = 21
          DataField = 'ESTOQOS'
          DataSource = DataSourceEstoque
          Enabled = False
          ReadOnly = True
          TabOrder = 1
          EditMask = ''
        end
        object escaninho: TJvDBMaskEdit
          Left = 237
          Top = 81
          Width = 39
          Height = 21
          Hint = 'Digite a localiza'#231#227'o'
          CharCase = ecUpperCase
          DataField = 'ESCANINHO_GAVETA'
          DataSource = DataSourceEstoque
          MaxLength = 2
          TabOrder = 5
          EditMask = 'cc;1;_'
          OnExit = escaninhoExit
          OnKeyPress = FormKeyPress
        end
        object Corredor: TJvDBMaskEdit
          Left = 85
          Top = 56
          Width = 39
          Height = 21
          Hint = 'Digite a localiza'#231#227'o'
          CharCase = ecUpperCase
          DataField = 'CORREDOR'
          DataSource = DataSourcePecasLocalizacao2
          Enabled = False
          MaxLength = 3
          TabOrder = 2
          EditMask = 'ccc;1;_'
          OnKeyPress = FormKeyPress
        end
        object Prateleira: TJvDBMaskEdit
          Left = 85
          Top = 81
          Width = 37
          Height = 21
          Hint = 'Digite a localiza'#231#227'o'
          CharCase = ecUpperCase
          DataField = 'PRATELEIRA'
          DataSource = DataSourceEstoque
          MaxLength = 3
          TabOrder = 4
          EditMask = 'ccc;1;_'
          OnKeyPress = FormKeyPress
        end
        object JvDBMaskEdit8: TJvDBMaskEdit
          Left = 237
          Top = 56
          Width = 42
          Height = 21
          Hint = 'Digite a localiza'#231#227'o'
          CharCase = ecUpperCase
          DataField = 'LOCAL'
          DataSource = DataSourceEstoque
          MaxLength = 5
          TabOrder = 3
          EditMask = 'ccccc;1;_'
          OnKeyPress = FormKeyPress
        end
      end
      object DBGrid5: TDBGrid
        Left = 589
        Top = 209
        Width = 315
        Height = 120
        DataSource = DataSourcePecasLocalizacao
        Enabled = False
        Options = [dgTitles, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
        ReadOnly = True
        TabOrder = 12
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        OnExit = DBGrid3Exit
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_PECAS'
            Title.Alignment = taCenter
            Title.Caption = 'Id Pe'#231'as:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'PRATELEIRA'
            Title.Alignment = taCenter
            Title.Caption = 'Prateleira:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ESCANINHO_GAVETA'
            Title.Alignment = taCenter
            Title.Caption = 'Escaninho:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CORREDOR'
            Title.Alignment = taCenter
            Title.Caption = 'Corredor:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'ID_CONCESSIONARIA'
            Title.Alignment = taCenter
            Title.Caption = 'Concess.:'
            Width = 51
            Visible = True
          end>
      end
      object JvDBMaskEdit1: TJvDBMaskEdit
        Left = 304
        Top = 30
        Width = 42
        Height = 21
        DataField = 'MIN_PEDIDO'
        DataSource = DataSourcePecas
        ReadOnly = True
        TabOrder = 4
        EditMask = ''
        OnKeyPress = FormKeyPress
      end
    end
    object TabSheetDadosEspecificos: TTabSheet
      Caption = 'Dados espec'#237'ficos'
      ImageIndex = 1
      object Label4: TLabel
        Left = 16
        Top = 6
        Width = 59
        Height = 13
        Caption = 'Custo atual:'
      end
      object Label5: TLabel
        Left = 16
        Top = 33
        Width = 63
        Height = 13
        Caption = 'Custo m'#233'dio:'
      end
      object Label6: TLabel
        Left = 16
        Top = 60
        Width = 22
        Height = 13
        Caption = 'PPS:'
      end
      object Label7: TLabel
        Left = 16
        Top = 87
        Width = 74
        Height = 13
        Caption = 'Pre'#231'o garantia:'
      end
      object Label8: TLabel
        Left = 16
        Top = 114
        Width = 71
        Height = 13
        Caption = #218'ltima compra:'
      end
      object Label9: TLabel
        Left = 16
        Top = 141
        Width = 66
        Height = 13
        Caption = #218'ltima venda:'
      end
      object Label10: TLabel
        Left = 16
        Top = 168
        Width = 68
        Height = 13
        Caption = #218'ltimo pedido:'
      end
      object Label18: TLabel
        Left = 200
        Top = 11
        Width = 74
        Height = 13
        Caption = 'Estoque M'#237'nimo'
      end
      object PPS: TJvDBMaskEdit
        Left = 97
        Top = 57
        Width = 88
        Height = 21
        DataField = 'PPS'
        DataSource = DataSourcePecas
        Enabled = False
        TabOrder = 2
        EditMask = ''
      end
      object PrecoGarantia: TJvDBMaskEdit
        Left = 97
        Top = 84
        Width = 88
        Height = 21
        DataField = 'PRECO_GAR'
        DataSource = DataSourcePecas
        Enabled = False
        TabOrder = 3
        EditMask = ''
      end
      object JvDBMaskEdit5: TJvDBMaskEdit
        Left = 97
        Top = 30
        Width = 88
        Height = 21
        DataField = 'CST_MEDIO'
        DataSource = DataSourceEstoque
        Enabled = False
        TabOrder = 1
        EditMask = ''
      end
      object JvDBMaskEdit10: TJvDBMaskEdit
        Left = 97
        Top = 3
        Width = 88
        Height = 21
        DataField = 'CST_ATUAL'
        DataSource = DataSourceEstoque
        Enabled = False
        TabOrder = 0
        EditMask = ''
      end
      object JvDBDateTimePicker1: TJvDBDateTimePicker
        Left = 97
        Top = 111
        Width = 88
        Height = 21
        Date = 41587.000000000000000000
        Time = 0.747711944437469400
        TabOrder = 4
        DropDownDate = 39793.000000000000000000
        DataField = 'ULT_COMPRA'
        DataSource = DataSourceEstoque
      end
      object JvDBDatePickerEdit1: TJvDBDatePickerEdit
        Left = 97
        Top = 138
        Width = 88
        Height = 21
        AllowNoDate = True
        DataField = 'ULT_VENDA'
        DataSource = DataSourceEstoque
        Enabled = False
        TabOrder = 5
      end
    end
    object TabSheetTributacao: TTabSheet
      Caption = 'Tributa'#231#227'o'
      ImageIndex = 3
      object IPILabel: TLabel
        Left = 17
        Top = 279
        Width = 94
        Height = 13
        Caption = 'Al'#237'quota de IPI(%):'
      end
      object ICMSLabel: TLabel
        Left = 17
        Top = 306
        Width = 82
        Height = 13
        Caption = 'Cr'#233'dito de ICMS:'
      end
      object Label11: TLabel
        Left = 16
        Top = 6
        Width = 68
        Height = 13
        Caption = 'NBM/SH-NCM:'
      end
      object ICMS: TJvDBMaskEdit
        Left = 103
        Top = 303
        Width = 121
        Height = 21
        DataField = 'ICMS'
        DataSource = DataSourcePecas
        Enabled = False
        TabOrder = 5
        EditMask = ''
        OnExit = ICMSExit
        OnKeyPress = FormKeyPress
      end
      object NBM: TJvDBMaskEdit
        Left = 90
        Top = 3
        Width = 75
        Height = 21
        DataField = 'NCM'
        DataSource = DataSourcePecas
        MaxLength = 10
        TabOrder = 0
        EditMask = '0000.00.00;1;_'
        OnKeyPress = FormKeyPress
      end
      object RadioGroup3: TRadioGroup
        Left = 17
        Top = 187
        Width = 296
        Height = 83
        Caption = 'Substitui'#231#227'o tribut'#225'ria do ICMS:'
        Items.Strings = (
          'Produto tributado integralmente'
          'ICMS cobrado anteriormente por substitui'#231#227'o tribut'#225'ria'
          'ICMS Subst.Tribut. - '#211'leo lubrificante')
        TabOrder = 3
      end
      object IPI: TJvDBMaskEdit
        Left = 117
        Top = 276
        Width = 107
        Height = 21
        DataField = 'IPI'
        DataSource = DataSourcePecas
        TabOrder = 4
        EditMask = ''
        OnExit = ICMSExit
        OnKeyPress = FormKeyPress
      end
      object DBRadioGroup1: TDBRadioGroup
        Left = 17
        Top = 30
        Width = 257
        Height = 151
        Caption = 'Origem da mercadoria:'
        DataField = 'COD_PROC'
        DataSource = DataSourcePecas
        Items.Strings = (
          '0.Nacional'
          '1.Estrangeira - importa'#231#227'o direta'
          '2.Estrangeira - adquirida no mercado interno'
          '3.Nacional - com conte'#250'do import.sup a 40%'
          '4.Nacional - decreto 288/67'
          '5.Nacional - com conte'#250'do import.inf a 40%'
          '6.Estrangeira-imp.dir.sem similar'
          '7.Estrangeira-imp.terc.sem similar')
        TabOrder = 1
        Values.Strings = (
          '0'
          '1'
          '2'
          '3'
          '4'
          '5'
          '6'
          '7')
      end
      object DBRadioGroup2: TDBRadioGroup
        Left = 312
        Top = 30
        Width = 257
        Height = 75
        Caption = 'Substitui'#231#227'o de PIS e COFINS'
        DataField = 'ST_COFINS'
        DataSource = DataSourcePecas
        Items.Strings = (
          'T - Com substitui'#231#227'o de PIS e COFINS'
          'F - Sem substitui'#231#227'o de PIS e COFINS')
        TabOrder = 2
        Values.Strings = (
          'T'
          'F')
      end
    end
    object TabSheetMovimentacao: TTabSheet
      Caption = 'Movimenta'#231#227'o'
      ImageIndex = 2
      object DBGrid1: TDBGrid
        Left = 3
        Top = 3
        Width = 677
        Height = 324
        DataSource = DataSourcePecasMovimentacao
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'ID_PECAS'
            Title.Alignment = taCenter
            Title.Caption = 'Sequ'#234'ncia:'
            Width = 63
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'MOVIMENTO'
            Title.Alignment = taCenter
            Title.Caption = 'Movimenta'#231#227'o:'
            Width = 73
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'QUANTIDADE_MOVIMENTADA'
            Title.Alignment = taCenter
            Title.Caption = 'Qtde Mov.:'
            Width = 58
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'QUANTIDADE_ATUAL'
            Title.Alignment = taCenter
            Title.Caption = 'Qtde Atual:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DATA'
            Title.Alignment = taCenter
            Title.Caption = 'Data:'
            Width = 83
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'EMISSAO'
            Title.Alignment = taRightJustify
            Title.Caption = 'Emiss'#227'o:'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CUSTO_MEDIO'
            Title.Alignment = taCenter
            Title.Caption = 'Custo m'#233'dio:'
            Width = 97
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'NF'
            Title.Alignment = taCenter
            Title.Caption = 'NF/OS/OP:'
            Width = 85
            Visible = True
          end>
      end
    end
  end
  object JvStatusBar1: TJvStatusBar
    Left = 0
    Top = 395
    Width = 910
    Height = 19
    Panels = <>
  end
  object ImageList1: TImageList
    Left = 456
    Top = 24
    Bitmap = {
      
      000000000000}
  end
  object DataSourcePecas: TDataSource
    AutoEdit = False
    DataSet = tblPecas
    Left = 304
    Top = 24
  end
  object tblPecas: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterOpen = tblPecasAfterOpen
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from Pecas'
      'where'
      '  ID_PECAS = :OLD_ID_PECAS')
    InsertSQL.Strings = (
      'insert into Pecas'
      
        '  (CFOP1, CFOP2, COD_PROC, COD_TRIB, CODIGO, CODIGO_SGS, CST, DE' +
        'SCRICAO, '
      
        '   ESPECIAL, FORA_LINHA, GRUPO, ICMS, ID_PECAS, IPI, LISTA, MIN_' +
        'PEDIDO, '
      
        '   NCM, NOVASUBS, ORIGINAL, PPS, PRECO, PRECO_GAR, QTD_PEDIDA, S' +
        'T_COFINS, '
      '   SUBST_TRIB, SUBSTITUTO, UNIDADE)'
      'values'
      
        '  (:CFOP1, :CFOP2, :COD_PROC, :COD_TRIB, :CODIGO, :CODIGO_SGS, :' +
        'CST, :DESCRICAO, '
      
        '   :ESPECIAL, :FORA_LINHA, :GRUPO, :ICMS, :ID_PECAS, :IPI, :LIST' +
        'A, :MIN_PEDIDO, '
      
        '   :NCM, :NOVASUBS, :ORIGINAL, :PPS, :PRECO, :PRECO_GAR, :QTD_PE' +
        'DIDA, :ST_COFINS, '
      '   :SUBST_TRIB, :SUBSTITUTO, :UNIDADE)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS,'
      '  CODIGO,'
      '  DESCRICAO,'
      '  GRUPO,'
      '  ESPECIAL,'
      '  COD_PROC,'
      '  COD_TRIB,'
      '  UNIDADE,'
      '  SUBSTITUTO,'
      '  FORA_LINHA,'
      '  MIN_PEDIDO,'
      '  QTD_PEDIDA,'
      '  ICMS,'
      '  PRECO,'
      '  PPS,'
      '  PRECO_GAR,'
      '  IPI,'
      '  LISTA,'
      '  SUBST_TRIB,'
      '  ST_COFINS,'
      '  NOVASUBS,'
      '  CFOP1,'
      '  CFOP2,'
      '  NCM,'
      '  ORIGINAL,'
      '  CST,'
      '  CODIGO_SGS'
      'from Pecas '
      'where'
      '  ID_PECAS = :ID_PECAS')
    SelectSQL.Strings = (
      'select * from Pecas'
      'where ID_Pecas = :id')
    ModifySQL.Strings = (
      'update Pecas'
      'set'
      '  CFOP1 = :CFOP1,'
      '  CFOP2 = :CFOP2,'
      '  COD_PROC = :COD_PROC,'
      '  COD_TRIB = :COD_TRIB,'
      '  CODIGO = :CODIGO,'
      '  CODIGO_SGS = :CODIGO_SGS,'
      '  CST = :CST,'
      '  DESCRICAO = :DESCRICAO,'
      '  ESPECIAL = :ESPECIAL,'
      '  FORA_LINHA = :FORA_LINHA,'
      '  GRUPO = :GRUPO,'
      '  ICMS = :ICMS,'
      '  ID_PECAS = :ID_PECAS,'
      '  IPI = :IPI,'
      '  LISTA = :LISTA,'
      '  MIN_PEDIDO = :MIN_PEDIDO,'
      '  NCM = :NCM,'
      '  NOVASUBS = :NOVASUBS,'
      '  ORIGINAL = :ORIGINAL,'
      '  PPS = :PPS,'
      '  PRECO = :PRECO,'
      '  PRECO_GAR = :PRECO_GAR,'
      '  QTD_PEDIDA = :QTD_PEDIDA,'
      '  ST_COFINS = :ST_COFINS,'
      '  SUBST_TRIB = :SUBST_TRIB,'
      '  SUBSTITUTO = :SUBSTITUTO,'
      '  UNIDADE = :UNIDADE'
      'where'
      '  ID_PECAS = :OLD_ID_PECAS')
    ParamCheck = True
    UniDirectional = False
    Left = 336
    Top = 24
    object tblPecasID_PECAS: TIntegerField
      FieldName = 'ID_PECAS'
      Origin = '"PECAS"."ID_PECAS"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object tblPecasCODIGO: TIBStringField
      FieldName = 'CODIGO'
      Origin = '"PECAS"."CODIGO"'
      Required = True
      FixedChar = True
      Size = 12
    end
    object tblPecasGRUPO: TIBStringField
      FieldName = 'GRUPO'
      Origin = '"PECAS"."GRUPO"'
      FixedChar = True
      Size = 3
    end
    object tblPecasESPECIAL: TIBStringField
      FieldName = 'ESPECIAL'
      Origin = '"PECAS"."ESPECIAL"'
      FixedChar = True
      Size = 1
    end
    object tblPecasCOD_PROC: TIBStringField
      FieldName = 'COD_PROC'
      Origin = '"PECAS"."COD_PROC"'
      FixedChar = True
      Size = 1
    end
    object tblPecasUNIDADE: TIBStringField
      FieldName = 'UNIDADE'
      Origin = '"PECAS"."UNIDADE"'
      FixedChar = True
      Size = 3
    end
    object tblPecasSUBSTITUTO: TIBStringField
      FieldName = 'SUBSTITUTO'
      Origin = '"PECAS"."SUBSTITUTO"'
      FixedChar = True
      Size = 12
    end
    object tblPecasFORA_LINHA: TIBStringField
      FieldName = 'FORA_LINHA'
      Origin = '"PECAS"."FORA_LINHA"'
      FixedChar = True
      Size = 1
    end
    object tblPecasMIN_PEDIDO: TIntegerField
      FieldName = 'MIN_PEDIDO'
      Origin = '"PECAS"."MIN_PEDIDO"'
    end
    object tblPecasQTD_PEDIDA: TIntegerField
      FieldName = 'QTD_PEDIDA'
      Origin = '"PECAS"."QTD_PEDIDA"'
    end
    object tblPecasICMS: TIBBCDField
      FieldName = 'ICMS'
      Origin = '"PECAS"."ICMS"'
      Precision = 9
      Size = 2
    end
    object tblPecasPRECO: TIBBCDField
      FieldName = 'PRECO'
      Origin = '"PECAS"."PRECO"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblPecasPRECO_GAR: TIBBCDField
      FieldName = 'PRECO_GAR'
      Origin = '"PECAS"."PRECO_GAR"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblPecasIPI: TIBBCDField
      FieldName = 'IPI'
      Origin = '"PECAS"."IPI"'
      Precision = 9
      Size = 2
    end
    object tblPecasLISTA: TIBStringField
      FieldName = 'LISTA'
      Origin = '"PECAS"."LISTA"'
      FixedChar = True
      Size = 1
    end
    object tblPecasSUBST_TRIB: TIBStringField
      FieldName = 'SUBST_TRIB'
      Origin = '"PECAS"."SUBST_TRIB"'
      FixedChar = True
      Size = 1
    end
    object tblPecasST_COFINS: TIBStringField
      FieldName = 'ST_COFINS'
      Origin = '"PECAS"."ST_COFINS"'
      FixedChar = True
      Size = 1
    end
    object tblPecasNOVASUBS: TIBStringField
      FieldName = 'NOVASUBS'
      Origin = '"PECAS"."NOVASUBS"'
      FixedChar = True
      Size = 1
    end
    object tblPecasCFOP1: TIBStringField
      FieldName = 'CFOP1'
      Origin = '"PECAS"."CFOP1"'
      FixedChar = True
      Size = 4
    end
    object tblPecasCFOP2: TIBStringField
      FieldName = 'CFOP2'
      Origin = '"PECAS"."CFOP2"'
      FixedChar = True
      Size = 4
    end
    object tblPecasNCM: TIBStringField
      FieldName = 'NCM'
      Origin = '"PECAS"."NCM"'
      Size = 10
    end
    object tblPecasPPS: TIBBCDField
      FieldName = 'PPS'
      Origin = '"PECAS"."PPS"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblPecasORIGINAL: TIBStringField
      FieldName = 'ORIGINAL'
      Origin = '"PECAS"."ORIGINAL"'
      FixedChar = True
      Size = 1
    end
    object tblPecasCST: TIBStringField
      FieldName = 'CST'
      Origin = '"PECAS"."CST"'
      Size = 3
    end
    object tblPecasCODIGO_SGS: TIntegerField
      FieldName = 'CODIGO_SGS'
      Origin = '"PECAS"."CODIGO_SGS"'
    end
    object tblPecasCOD_TRIB: TIBStringField
      FieldName = 'COD_TRIB'
      Origin = '"PECAS"."COD_TRIB"'
      FixedChar = True
      Size = 2
    end
    object tblPecasDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = '"PECAS"."DESCRICAO"'
      Required = True
      Size = 60
    end
  end
  object ActionList1: TActionList
    Images = ImageList1
    Left = 424
    Top = 24
    object Procurar: TAction
      Caption = 'F4-&Procurar'
      ShortCut = 115
      OnExecute = ProcurarExecute
    end
    object Consultar: TAction
      Caption = 'F5-C&onsultar'
      ShortCut = 116
      OnExecute = ConsultarExecute
    end
    object Novo: TAction
      Caption = 'F6-&Novo'
      ShortCut = 117
      OnExecute = NovoExecute
    end
    object Editar: TAction
      Caption = 'F7-&Editar'
      ShortCut = 118
      OnExecute = EditarExecute
    end
    object Excluir: TAction
      Caption = 'F8-E&xcluir'
      ShortCut = 119
      OnExecute = ExcluirExecute
    end
    object Cancelar: TAction
      Caption = 'F9-&Cancelar'
      ShortCut = 120
      OnExecute = CancelarExecute
    end
    object Salvar: TAction
      Caption = 'F10-&Salvar'
      ShortCut = 121
      OnExecute = SalvarExecute
    end
  end
  object tblPecasMovimentacao: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from PECAS_MOVIMENTACAO'
      'where'
      '  ID_PECAS_MOVIMENTACAO = :OLD_ID_PECAS_MOVIMENTACAO')
    InsertSQL.Strings = (
      'insert into PECAS_MOVIMENTACAO'
      
        '  (CUSTO_MEDIO, DATA, EMISSAO, ID_CONCESSIONARIA, ID_PECAS, ID_P' +
        'ECAS_MOVIMENTACAO, '
      '   MOVIMENTO, NF, QUANTIDADE_ATUAL, QUANTIDADE_MOVIMENTADA)'
      'values'
      
        '  (:CUSTO_MEDIO, :DATA, :EMISSAO, :ID_CONCESSIONARIA, :ID_PECAS,' +
        ' :ID_PECAS_MOVIMENTACAO, '
      '   :MOVIMENTO, :NF, :QUANTIDADE_ATUAL, :QUANTIDADE_MOVIMENTADA)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS_MOVIMENTACAO,'
      '  ID_CONCESSIONARIA,'
      '  ID_PECAS,'
      '  MOVIMENTO,'
      '  QUANTIDADE_MOVIMENTADA,'
      '  QUANTIDADE_ATUAL,'
      '  CUSTO_MEDIO,'
      '  DATA,'
      '  NF,'
      '  EMISSAO'
      'from PECAS_MOVIMENTACAO '
      'where'
      '  ID_PECAS_MOVIMENTACAO = :ID_PECAS_MOVIMENTACAO')
    SelectSQL.Strings = (
      'select * from PECAS_MOVIMENTACAO'
      'Where ID_PECAS = :Id'
      'and'
      'id_Concessionaria = :idConcessionaria'
      'order by data, ID_PECAS_MOVIMENTACAO')
    ModifySQL.Strings = (
      'update PECAS_MOVIMENTACAO'
      'set'
      '  CUSTO_MEDIO = :CUSTO_MEDIO,'
      '  DATA = :DATA,'
      '  EMISSAO = :EMISSAO,'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA,'
      '  ID_PECAS = :ID_PECAS,'
      '  ID_PECAS_MOVIMENTACAO = :ID_PECAS_MOVIMENTACAO,'
      '  MOVIMENTO = :MOVIMENTO,'
      '  NF = :NF,'
      '  QUANTIDADE_ATUAL = :QUANTIDADE_ATUAL,'
      '  QUANTIDADE_MOVIMENTADA = :QUANTIDADE_MOVIMENTADA'
      'where'
      '  ID_PECAS_MOVIMENTACAO = :OLD_ID_PECAS_MOVIMENTACAO')
    ParamCheck = True
    UniDirectional = False
    Left = 672
    Top = 56
    object tblPecasMovimentacaoID_PECAS_MOVIMENTACAO: TIntegerField
      FieldName = 'ID_PECAS_MOVIMENTACAO'
      Origin = '"PECAS_MOVIMENTACAO"."ID_PECAS_MOVIMENTACAO"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object tblPecasMovimentacaoID_CONCESSIONARIA: TIntegerField
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"PECAS_MOVIMENTACAO"."ID_CONCESSIONARIA"'
      Required = True
    end
    object tblPecasMovimentacaoID_PECAS: TIntegerField
      FieldName = 'ID_PECAS'
      Origin = '"PECAS_MOVIMENTACAO"."ID_PECAS"'
      Required = True
    end
    object tblPecasMovimentacaoMOVIMENTO: TIBStringField
      FieldName = 'MOVIMENTO'
      Origin = '"PECAS_MOVIMENTACAO"."MOVIMENTO"'
      Size = 1
    end
    object tblPecasMovimentacaoQUANTIDADE_MOVIMENTADA: TIntegerField
      FieldName = 'QUANTIDADE_MOVIMENTADA'
      Origin = '"PECAS_MOVIMENTACAO"."QUANTIDADE_MOVIMENTADA"'
    end
    object tblPecasMovimentacaoQUANTIDADE_ATUAL: TIntegerField
      FieldName = 'QUANTIDADE_ATUAL'
      Origin = '"PECAS_MOVIMENTACAO"."QUANTIDADE_ATUAL"'
    end
    object tblPecasMovimentacaoCUSTO_MEDIO: TIBBCDField
      FieldName = 'CUSTO_MEDIO'
      Origin = '"PECAS_MOVIMENTACAO"."CUSTO_MEDIO"'
      Precision = 18
      Size = 2
    end
    object tblPecasMovimentacaoDATA: TDateField
      FieldName = 'DATA'
      Origin = '"PECAS_MOVIMENTACAO"."DATA"'
    end
    object tblPecasMovimentacaoNF: TIBStringField
      DisplayLabel = 'NF/ORC/OS'
      FieldName = 'NF'
      Origin = '"PECAS_MOVIMENTACAO"."NF"'
      Size = 8
    end
    object tblPecasMovimentacaoEMISSAO: TDateField
      FieldName = 'EMISSAO'
      Origin = '"PECAS_MOVIMENTACAO"."EMISSAO"'
    end
  end
  object DataSourcePecasMovimentacao: TDataSource
    AutoEdit = False
    DataSet = tblPecasMovimentacao
    Left = 640
    Top = 56
  end
  object DataSourcePecasEstoque: TDataSource
    AutoEdit = False
    DataSet = tblPecasEstoque
    Left = 200
    Top = 296
  end
  object tblPecasEstoque: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from PECAS_ESTOQUE'
      'where'
      '  ID_PECAS_ESTOQUE = :OLD_ID_PECAS_ESTOQUE')
    InsertSQL.Strings = (
      'insert into PECAS_ESTOQUE'
      
        '  (CST_ATUAL, CST_MEDIO, ESTOQOS, ESTOQUE, FORNECEDOR, ID_CONCES' +
        'SIONARIA, '
      
        '   ID_PECAS, ID_PECAS_ESTOQUE, LOCAL, ULT_COMPRA, ULT_PEDIDO, UL' +
        'T_VENDA)'
      'values'
      
        '  (:CST_ATUAL, :CST_MEDIO, :ESTOQOS, :ESTOQUE, :FORNECEDOR, :ID_' +
        'CONCESSIONARIA, '
      
        '   :ID_PECAS, :ID_PECAS_ESTOQUE, :LOCAL, :ULT_COMPRA, :ULT_PEDID' +
        'O, :ULT_VENDA)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS_ESTOQUE,'
      '  ID_PECAS,'
      '  ID_CONCESSIONARIA,'
      '  ESTOQUE,'
      '  ESTOQOS,'
      '  LOCAL,'
      '  CST_MEDIO,'
      '  CST_ATUAL,'
      '  ULT_COMPRA,'
      '  ULT_VENDA,'
      '  ULT_PEDIDO,'
      '  FORNECEDOR'
      'from PECAS_ESTOQUE '
      'where'
      '  ID_PECAS_ESTOQUE = :ID_PECAS_ESTOQUE')
    SelectSQL.Strings = (
      'select P.* , C.EMPRESA'
      ''
      'from PECAS_ESTOQUE P'
      ''
      'LEFT JOIN CONCESSIONARIA C ON C.ID_CONCESSIONARIA ='
      'P.ID_CONCESSIONARIA'
      ''
      'Where P.ID_Pecas = :Id')
    ModifySQL.Strings = (
      'update PECAS_ESTOQUE'
      'set'
      '  CST_ATUAL = :CST_ATUAL,'
      '  CST_MEDIO = :CST_MEDIO,'
      '  ESTOQOS = :ESTOQOS,'
      '  ESTOQUE = :ESTOQUE,'
      '  FORNECEDOR = :FORNECEDOR,'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA,'
      '  ID_PECAS = :ID_PECAS,'
      '  ID_PECAS_ESTOQUE = :ID_PECAS_ESTOQUE,'
      '  LOCAL = :LOCAL,'
      '  ULT_COMPRA = :ULT_COMPRA,'
      '  ULT_PEDIDO = :ULT_PEDIDO,'
      '  ULT_VENDA = :ULT_VENDA'
      'where'
      '  ID_PECAS_ESTOQUE = :OLD_ID_PECAS_ESTOQUE')
    ParamCheck = True
    UniDirectional = False
    Left = 232
    Top = 296
    object tblPecasEstoqueID_PECAS: TIntegerField
      FieldName = 'ID_PECAS'
      Origin = '"PECAS_ESTOQUE"."ID_PECAS"'
    end
    object tblPecasEstoqueID_CONCESSIONARIA: TIntegerField
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"PECAS_ESTOQUE"."ID_CONCESSIONARIA"'
    end
    object tblPecasEstoqueESTOQUE: TIntegerField
      FieldName = 'ESTOQUE'
      Origin = '"PECAS_ESTOQUE"."ESTOQUE"'
    end
    object tblPecasEstoqueESTOQOS: TIntegerField
      FieldName = 'ESTOQOS'
      Origin = '"PECAS_ESTOQUE"."ESTOQOS"'
    end
    object tblPecasEstoqueLOCAL: TIBStringField
      FieldName = 'LOCAL'
      Origin = '"PECAS_ESTOQUE"."LOCAL"'
      FixedChar = True
      Size = 5
    end
    object tblPecasEstoqueEMPRESA: TIBStringField
      FieldName = 'EMPRESA'
      Origin = '"CONCESSIONARIA"."EMPRESA"'
      Size = 50
    end
  end
  object DataSourceEstoque: TDataSource
    DataSet = tblestoque
    Left = 336
    Top = 192
  end
  object tblestoque: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from PECAS_ESTOQUE'
      'where'
      '  ID_PECAS_ESTOQUE = :OLD_ID_PECAS_ESTOQUE')
    InsertSQL.Strings = (
      'insert into PECAS_ESTOQUE'
      
        '  (CST_ATUAL, CST_MEDIO, ESCANINHO_GAVETA, ESTOQOS, ESTOQUE, FOR' +
        'NECEDOR, '
      
        '   ID_CONCESSIONARIA, ID_PECAS, ID_PECAS_ESTOQUE, LOCAL, PRATELE' +
        'IRA, ULT_COMPRA, '
      '   ULT_PEDIDO, ULT_VENDA)'
      'values'
      
        '  (:CST_ATUAL, :CST_MEDIO, :ESCANINHO_GAVETA, :ESTOQOS, :ESTOQUE' +
        ', :FORNECEDOR, '
      
        '   :ID_CONCESSIONARIA, :ID_PECAS, :ID_PECAS_ESTOQUE, :LOCAL, :PR' +
        'ATELEIRA, '
      '   :ULT_COMPRA, :ULT_PEDIDO, :ULT_VENDA)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS_ESTOQUE,'
      '  ID_PECAS,'
      '  ID_CONCESSIONARIA,'
      '  ESTOQUE,'
      '  ESTOQOS,'
      '  LOCAL,'
      '  CST_MEDIO,'
      '  CST_ATUAL,'
      '  ULT_COMPRA,'
      '  ULT_VENDA,'
      '  ULT_PEDIDO,'
      '  FORNECEDOR,'
      '  PRATELEIRA,'
      '  ESCANINHO_GAVETA'
      'from PECAS_ESTOQUE '
      'where'
      '  ID_PECAS_ESTOQUE = :ID_PECAS_ESTOQUE')
    SelectSQL.Strings = (
      'select P.* from PECAS_ESTOQUE P'
      ''
      'Where (P.id_pecas = :id) and'
      '(P.ID_Concessionaria = :id_concessionaria)')
    ModifySQL.Strings = (
      'update PECAS_ESTOQUE'
      'set'
      '  CST_ATUAL = :CST_ATUAL,'
      '  CST_MEDIO = :CST_MEDIO,'
      '  ESCANINHO_GAVETA = :ESCANINHO_GAVETA,'
      '  ESTOQOS = :ESTOQOS,'
      '  ESTOQUE = :ESTOQUE,'
      '  FORNECEDOR = :FORNECEDOR,'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA,'
      '  ID_PECAS = :ID_PECAS,'
      '  ID_PECAS_ESTOQUE = :ID_PECAS_ESTOQUE,'
      '  LOCAL = :LOCAL,'
      '  PRATELEIRA = :PRATELEIRA,'
      '  ULT_COMPRA = :ULT_COMPRA,'
      '  ULT_PEDIDO = :ULT_PEDIDO,'
      '  ULT_VENDA = :ULT_VENDA'
      'where'
      '  ID_PECAS_ESTOQUE = :OLD_ID_PECAS_ESTOQUE')
    ParamCheck = True
    UniDirectional = False
    Left = 368
    Top = 192
    object tblestoqueID_PECAS: TIntegerField
      FieldName = 'ID_PECAS'
      Origin = '"PECAS_ESTOQUE"."ID_PECAS"'
      Required = True
    end
    object tblestoqueID_CONCESSIONARIA: TIntegerField
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"PECAS_ESTOQUE"."ID_CONCESSIONARIA"'
      Required = True
    end
    object tblestoqueESTOQUE: TIntegerField
      FieldName = 'ESTOQUE'
      Origin = '"PECAS_ESTOQUE"."ESTOQUE"'
    end
    object tblestoqueESTOQOS: TIntegerField
      FieldName = 'ESTOQOS'
      Origin = '"PECAS_ESTOQUE"."ESTOQOS"'
    end
    object tblestoqueLOCAL: TIBStringField
      FieldName = 'LOCAL'
      Origin = '"PECAS_ESTOQUE"."LOCAL"'
      FixedChar = True
      Size = 5
    end
    object tblestoqueID_PECAS_ESTOQUE: TIntegerField
      FieldName = 'ID_PECAS_ESTOQUE'
      Origin = '"PECAS_ESTOQUE"."ID_PECAS_ESTOQUE"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object tblestoqueCST_MEDIO: TIBBCDField
      FieldName = 'CST_MEDIO'
      Origin = '"PECAS_ESTOQUE"."CST_MEDIO"'
      Precision = 18
      Size = 2
    end
    object tblestoqueCST_ATUAL: TIBBCDField
      FieldName = 'CST_ATUAL'
      Origin = '"PECAS_ESTOQUE"."CST_ATUAL"'
      Precision = 18
      Size = 2
    end
    object tblestoqueULT_COMPRA: TDateField
      FieldName = 'ULT_COMPRA'
      Origin = '"PECAS_ESTOQUE"."ULT_COMPRA"'
    end
    object tblestoqueULT_VENDA: TDateField
      FieldName = 'ULT_VENDA'
      Origin = '"PECAS_ESTOQUE"."ULT_VENDA"'
    end
    object tblestoqueULT_PEDIDO: TDateField
      FieldName = 'ULT_PEDIDO'
      Origin = '"PECAS_ESTOQUE"."ULT_PEDIDO"'
    end
    object tblestoqueFORNECEDOR: TIBStringField
      FieldName = 'FORNECEDOR'
      Origin = '"PECAS_ESTOQUE"."FORNECEDOR"'
      FixedChar = True
      Size = 5
    end
    object tblestoquePRATELEIRA: TIBStringField
      FieldName = 'PRATELEIRA'
      Origin = '"PECAS_ESTOQUE"."PRATELEIRA"'
      Size = 3
    end
    object tblestoqueESCANINHO_GAVETA: TIBStringField
      FieldName = 'ESCANINHO_GAVETA'
      Origin = '"PECAS_ESTOQUE"."ESCANINHO_GAVETA"'
      Size = 2
    end
  end
  object IBQuery1: TIBQuery
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    BufferChunks = 1000
    CachedUpdates = False
    ParamCheck = True
    SQL.Strings = (
      'SELECT GEN_ID(Seq_Pecas_Id, 1) as'
      'Id_Pecas From RDB$DATABASE')
    Left = 368
    Top = 24
  end
  object tblPecasFabrica: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from PECYAM'
      'where'
      '  ID_PECYAM = :OLD_ID_PECYAM')
    InsertSQL.Strings = (
      'insert into PECYAM'
      
        '  (CODIGO, CODIGO_NOVO, CUSTO, DESCRICAO, ID_PECYAM, IPI, ORIGEM' +
        ', PESO, '
      '   PPS, PRECO_GARAN, QTDE_EMBAL, REAJUSTE, RESERVADO)'
      'values'
      
        '  (:CODIGO, :CODIGO_NOVO, :CUSTO, :DESCRICAO, :ID_PECYAM, :IPI, ' +
        ':ORIGEM, '
      
        '   :PESO, :PPS, :PRECO_GARAN, :QTDE_EMBAL, :REAJUSTE, :RESERVADO' +
        ')')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECYAM,'
      '  CODIGO,'
      '  DESCRICAO,'
      '  CODIGO_NOVO,'
      '  ORIGEM,'
      '  CUSTO,'
      '  PESO,'
      '  QTDE_EMBAL,'
      '  IPI,'
      '  REAJUSTE,'
      '  PRECO_GARAN,'
      '  PPS,'
      '  RESERVADO'
      'from PECYAM '
      'where'
      '  ID_PECYAM = :ID_PECYAM')
    SelectSQL.Strings = (
      'select * from  PECYAM'
      'where Codigo = :IdCodigo')
    ModifySQL.Strings = (
      'update PECYAM'
      'set'
      '  CODIGO = :CODIGO,'
      '  CODIGO_NOVO = :CODIGO_NOVO,'
      '  CUSTO = :CUSTO,'
      '  DESCRICAO = :DESCRICAO,'
      '  ID_PECYAM = :ID_PECYAM,'
      '  IPI = :IPI,'
      '  ORIGEM = :ORIGEM,'
      '  PESO = :PESO,'
      '  PPS = :PPS,'
      '  PRECO_GARAN = :PRECO_GARAN,'
      '  QTDE_EMBAL = :QTDE_EMBAL,'
      '  REAJUSTE = :REAJUSTE,'
      '  RESERVADO = :RESERVADO'
      'where'
      '  ID_PECYAM = :OLD_ID_PECYAM')
    ParamCheck = True
    UniDirectional = False
    Left = 336
    Top = 56
    object tblPecasFabricaID_PECYAM: TIntegerField
      FieldName = 'ID_PECYAM'
      Origin = '"PECYAM"."ID_PECYAM"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object tblPecasFabricaCODIGO: TIBStringField
      FieldName = 'CODIGO'
      Origin = '"PECYAM"."CODIGO"'
      Size = 12
    end
    object tblPecasFabricaDESCRICAO: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = '"PECYAM"."DESCRICAO"'
      Size = 50
    end
    object tblPecasFabricaCODIGO_NOVO: TIBStringField
      FieldName = 'CODIGO_NOVO'
      Origin = '"PECYAM"."CODIGO_NOVO"'
      Size = 12
    end
    object tblPecasFabricaORIGEM: TIBStringField
      FieldName = 'ORIGEM'
      Origin = '"PECYAM"."ORIGEM"'
      Size = 1
    end
    object tblPecasFabricaCUSTO: TIBBCDField
      FieldName = 'CUSTO'
      Origin = '"PECYAM"."CUSTO"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblPecasFabricaQTDE_EMBAL: TIBStringField
      FieldName = 'QTDE_EMBAL'
      Origin = '"PECYAM"."QTDE_EMBAL"'
      Size = 5
    end
    object tblPecasFabricaPESO: TIBStringField
      FieldName = 'PESO'
      Origin = '"PECYAM"."PESO"'
      Size = 7
    end
    object tblPecasFabricaIPI: TIBBCDField
      FieldName = 'IPI'
      Origin = '"PECYAM"."IPI"'
      Precision = 9
      Size = 2
    end
    object tblPecasFabricaREAJUSTE: TIBBCDField
      FieldName = 'REAJUSTE'
      Origin = '"PECYAM"."REAJUSTE"'
      Precision = 9
      Size = 2
    end
    object tblPecasFabricaPRECO_GARAN: TIBBCDField
      FieldName = 'PRECO_GARAN'
      Origin = '"PECYAM"."PRECO_GARAN"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblPecasFabricaPPS: TIBBCDField
      FieldName = 'PPS'
      Origin = '"PECYAM"."PPS"'
      currency = True
      Precision = 18
      Size = 2
    end
    object tblPecasFabricaRESERVADO: TIBStringField
      FieldName = 'RESERVADO'
      Origin = '"PECYAM"."RESERVADO"'
      Size = 2
    end
  end
  object DataSourcePecasFabrica: TDataSource
    AutoEdit = False
    DataSet = tblPecasFabrica
    Left = 304
    Top = 56
  end
  object DataSourcePecas2: TDataSource
    AutoEdit = False
    DataSet = tblPecas2
    Left = 304
    Top = 96
  end
  object tblPecas2: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from Pecas'
      'where'
      '  ID_PECAS = :OLD_ID_PECAS')
    InsertSQL.Strings = (
      'insert into Pecas'
      
        '  (CFOP1, CFOP2, COD_PROC, COD_TRIB, CODIGO, CST, DESCRICAO, ESP' +
        'ECIAL, '
      
        '   FORA_LINHA, GRUPO, ICMS, ID_PECAS, IPI, LISTA, MIN_PEDIDO, NC' +
        'M, NOVASUBS, '
      
        '   ORIGINAL, PPS, PRECO, PRECO_GAR, QTD_PEDIDA, ST_COFINS, SUBST' +
        '_TRIB, '
      '   SUBSTITUTO, UNIDADE)'
      'values'
      
        '  (:CFOP1, :CFOP2, :COD_PROC, :COD_TRIB, :CODIGO, :CST, :DESCRIC' +
        'AO, :ESPECIAL, '
      
        '   :FORA_LINHA, :GRUPO, :ICMS, :ID_PECAS, :IPI, :LISTA, :MIN_PED' +
        'IDO, :NCM, '
      
        '   :NOVASUBS, :ORIGINAL, :PPS, :PRECO, :PRECO_GAR, :QTD_PEDIDA, ' +
        ':ST_COFINS, '
      '   :SUBST_TRIB, :SUBSTITUTO, :UNIDADE)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS,'
      '  CODIGO,'
      '  DESCRICAO,'
      '  GRUPO,'
      '  ESPECIAL,'
      '  COD_PROC,'
      '  COD_TRIB,'
      '  UNIDADE,'
      '  SUBSTITUTO,'
      '  FORA_LINHA,'
      '  MIN_PEDIDO,'
      '  QTD_PEDIDA,'
      '  ICMS,'
      '  PRECO,'
      '  PPS,'
      '  PRECO_GAR,'
      '  IPI,'
      '  LISTA,'
      '  SUBST_TRIB,'
      '  ST_COFINS,'
      '  NOVASUBS,'
      '  CFOP1,'
      '  CFOP2,'
      '  NCM,'
      '  ORIGINAL,'
      '  CST'
      'from Pecas '
      'where'
      '  ID_PECAS = :ID_PECAS')
    SelectSQL.Strings = (
      'select * from Pecas'
      'where CODIGO = :idcodigo')
    ModifySQL.Strings = (
      'update Pecas'
      'set'
      '  CFOP1 = :CFOP1,'
      '  CFOP2 = :CFOP2,'
      '  COD_PROC = :COD_PROC,'
      '  COD_TRIB = :COD_TRIB,'
      '  CODIGO = :CODIGO,'
      '  CST = :CST,'
      '  DESCRICAO = :DESCRICAO,'
      '  ESPECIAL = :ESPECIAL,'
      '  FORA_LINHA = :FORA_LINHA,'
      '  GRUPO = :GRUPO,'
      '  ICMS = :ICMS,'
      '  ID_PECAS = :ID_PECAS,'
      '  IPI = :IPI,'
      '  LISTA = :LISTA,'
      '  MIN_PEDIDO = :MIN_PEDIDO,'
      '  NCM = :NCM,'
      '  NOVASUBS = :NOVASUBS,'
      '  ORIGINAL = :ORIGINAL,'
      '  PPS = :PPS,'
      '  PRECO = :PRECO,'
      '  PRECO_GAR = :PRECO_GAR,'
      '  QTD_PEDIDA = :QTD_PEDIDA,'
      '  ST_COFINS = :ST_COFINS,'
      '  SUBST_TRIB = :SUBST_TRIB,'
      '  SUBSTITUTO = :SUBSTITUTO,'
      '  UNIDADE = :UNIDADE'
      'where'
      '  ID_PECAS = :OLD_ID_PECAS')
    ParamCheck = True
    UniDirectional = False
    Left = 336
    Top = 96
    object IntegerField1: TIntegerField
      FieldName = 'ID_PECAS'
      Origin = '"PECAS"."ID_PECAS"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object IBStringField1: TIBStringField
      FieldName = 'CODIGO'
      Origin = '"PECAS"."CODIGO"'
      Required = True
      FixedChar = True
      Size = 12
    end
    object IBStringField2: TIBStringField
      FieldName = 'DESCRICAO'
      Origin = '"PECAS"."DESCRICAO"'
      Required = True
      Size = 50
    end
    object IBStringField3: TIBStringField
      FieldName = 'GRUPO'
      Origin = '"PECAS"."GRUPO"'
      FixedChar = True
      Size = 3
    end
    object IBStringField4: TIBStringField
      FieldName = 'ESPECIAL'
      Origin = '"PECAS"."ESPECIAL"'
      FixedChar = True
      Size = 1
    end
    object IBStringField5: TIBStringField
      FieldName = 'COD_PROC'
      Origin = '"PECAS"."COD_PROC"'
      FixedChar = True
      Size = 1
    end
    object IBStringField6: TIBStringField
      FieldName = 'COD_TRIB'
      Origin = '"PECAS"."COD_TRIB"'
      FixedChar = True
      Size = 2
    end
    object IBStringField7: TIBStringField
      FieldName = 'UNIDADE'
      Origin = '"PECAS"."UNIDADE"'
      FixedChar = True
      Size = 3
    end
    object IBStringField8: TIBStringField
      FieldName = 'SUBSTITUTO'
      Origin = '"PECAS"."SUBSTITUTO"'
      FixedChar = True
      Size = 12
    end
    object IBStringField9: TIBStringField
      FieldName = 'FORA_LINHA'
      Origin = '"PECAS"."FORA_LINHA"'
      FixedChar = True
      Size = 1
    end
    object IntegerField2: TIntegerField
      FieldName = 'MIN_PEDIDO'
      Origin = '"PECAS"."MIN_PEDIDO"'
    end
    object IntegerField3: TIntegerField
      FieldName = 'QTD_PEDIDA'
      Origin = '"PECAS"."QTD_PEDIDA"'
    end
    object IBBCDField1: TIBBCDField
      FieldName = 'ICMS'
      Origin = '"PECAS"."ICMS"'
      Precision = 9
      Size = 2
    end
    object IBBCDField2: TIBBCDField
      FieldName = 'PRECO'
      Origin = '"PECAS"."PRECO"'
      currency = True
      Precision = 18
      Size = 2
    end
    object IBBCDField3: TIBBCDField
      FieldName = 'PRECO_GAR'
      Origin = '"PECAS"."PRECO_GAR"'
      currency = True
      Precision = 18
      Size = 2
    end
    object IBBCDField4: TIBBCDField
      FieldName = 'IPI'
      Origin = '"PECAS"."IPI"'
      Precision = 9
      Size = 2
    end
    object IBStringField10: TIBStringField
      FieldName = 'LISTA'
      Origin = '"PECAS"."LISTA"'
      FixedChar = True
      Size = 1
    end
    object IBStringField11: TIBStringField
      FieldName = 'SUBST_TRIB'
      Origin = '"PECAS"."SUBST_TRIB"'
      FixedChar = True
      Size = 1
    end
    object IBStringField12: TIBStringField
      FieldName = 'ST_COFINS'
      Origin = '"PECAS"."ST_COFINS"'
      FixedChar = True
      Size = 1
    end
    object IBStringField13: TIBStringField
      FieldName = 'NOVASUBS'
      Origin = '"PECAS"."NOVASUBS"'
      FixedChar = True
      Size = 1
    end
    object IBStringField14: TIBStringField
      FieldName = 'CFOP1'
      Origin = '"PECAS"."CFOP1"'
      FixedChar = True
      Size = 4
    end
    object IBStringField15: TIBStringField
      FieldName = 'CFOP2'
      Origin = '"PECAS"."CFOP2"'
      FixedChar = True
      Size = 4
    end
    object IBStringField16: TIBStringField
      FieldName = 'NCM'
      Origin = '"PECAS"."NCM"'
      Size = 10
    end
    object IBBCDField5: TIBBCDField
      FieldName = 'PPS'
      Origin = '"PECAS"."PPS"'
      currency = True
      Precision = 18
      Size = 2
    end
    object IBStringField17: TIBStringField
      FieldName = 'ORIGINAL'
      Origin = '"PECAS"."ORIGINAL"'
      FixedChar = True
      Size = 1
    end
    object IBStringField18: TIBStringField
      FieldName = 'CST'
      Origin = '"PECAS"."CST"'
      Size = 3
    end
  end
  object DataSourcePecasLocalizacao: TDataSource
    AutoEdit = False
    DataSet = tblPecasLocalizacao
    Left = 456
    Top = 312
  end
  object tblPecasLocalizacao: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from Pecas_Localizacao'
      'where'
      '  ID_PECAS_LOCALIZACAO = :OLD_ID_PECAS_LOCALIZACAO')
    InsertSQL.Strings = (
      'insert into Pecas_Localizacao'
      
        '  (CODIGO_SGS, CORREDOR, ESCANINHO_GAVETA, ID_CONCESSIONARIA, ID' +
        '_PECAS, '
      '   ID_PECAS_LOCALIZACAO, PRATELEIRA)'
      'values'
      
        '  (:CODIGO_SGS, :CORREDOR, :ESCANINHO_GAVETA, :ID_CONCESSIONARIA' +
        ', :ID_PECAS, '
      '   :ID_PECAS_LOCALIZACAO, :PRATELEIRA)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS_LOCALIZACAO,'
      '  ID_CONCESSIONARIA,'
      '  ID_PECAS,'
      '  PRATELEIRA,'
      '  ESCANINHO_GAVETA,'
      '  CORREDOR,'
      '  CODIGO_SGS'
      'from Pecas_Localizacao '
      'where'
      '  ID_PECAS_LOCALIZACAO = :ID_PECAS_LOCALIZACAO')
    SelectSQL.Strings = (
      'select * from  Pecas_Localizacao'
      'where Id_Pecas = :IdPecas')
    ModifySQL.Strings = (
      'update Pecas_Localizacao'
      'set'
      '  CODIGO_SGS = :CODIGO_SGS,'
      '  CORREDOR = :CORREDOR,'
      '  ESCANINHO_GAVETA = :ESCANINHO_GAVETA,'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA,'
      '  ID_PECAS = :ID_PECAS,'
      '  ID_PECAS_LOCALIZACAO = :ID_PECAS_LOCALIZACAO,'
      '  PRATELEIRA = :PRATELEIRA'
      'where'
      '  ID_PECAS_LOCALIZACAO = :OLD_ID_PECAS_LOCALIZACAO')
    ParamCheck = True
    UniDirectional = False
    Left = 488
    Top = 312
    object tblPecasLocalizacaoID_PECAS_LOCALIZACAO: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_PECAS_LOCALIZACAO'
      Origin = '"PECAS_LOCALIZACAO"."ID_PECAS_LOCALIZACAO"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object tblPecasLocalizacaoID_CONCESSIONARIA: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"PECAS_LOCALIZACAO"."ID_CONCESSIONARIA"'
    end
    object tblPecasLocalizacaoID_PECAS: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_PECAS'
      Origin = '"PECAS_LOCALIZACAO"."ID_PECAS"'
    end
    object tblPecasLocalizacaoPRATELEIRA: TIBStringField
      DisplayWidth = 3
      FieldName = 'PRATELEIRA'
      Origin = '"PECAS_LOCALIZACAO"."PRATELEIRA"'
      Size = 3
    end
    object tblPecasLocalizacaoESCANINHO_GAVETA: TIBStringField
      DisplayWidth = 2
      FieldName = 'ESCANINHO_GAVETA'
      Origin = '"PECAS_LOCALIZACAO"."ESCANINHO_GAVETA"'
      Size = 2
    end
    object tblPecasLocalizacaoCORREDOR: TIBStringField
      DisplayWidth = 3
      FieldName = 'CORREDOR'
      Origin = '"PECAS_LOCALIZACAO"."CORREDOR"'
      Size = 3
    end
    object tblPecasLocalizacaoCODIGO_SGS: TIntegerField
      DisplayWidth = 10
      FieldName = 'CODIGO_SGS'
      Origin = '"PECAS_LOCALIZACAO"."CODIGO_SGS"'
    end
  end
  object tblPecasLocalizacao2: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from Pecas_Localizacao'
      'where'
      '  ID_PECAS_LOCALIZACAO = :OLD_ID_PECAS_LOCALIZACAO')
    InsertSQL.Strings = (
      'insert into Pecas_Localizacao'
      
        '  (CODIGO_SGS, CORREDOR, ESCANINHO_GAVETA, ID_CONCESSIONARIA, ID' +
        '_PECAS, '
      '   ID_PECAS_LOCALIZACAO, PRATELEIRA)'
      'values'
      
        '  (:CODIGO_SGS, :CORREDOR, :ESCANINHO_GAVETA, :ID_CONCESSIONARIA' +
        ', :ID_PECAS, '
      '   :ID_PECAS_LOCALIZACAO, :PRATELEIRA)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS_LOCALIZACAO,'
      '  ID_CONCESSIONARIA,'
      '  ID_PECAS,'
      '  PRATELEIRA,'
      '  ESCANINHO_GAVETA,'
      '  CORREDOR,'
      '  CODIGO_SGS'
      'from Pecas_Localizacao '
      'where'
      '  ID_PECAS_LOCALIZACAO = :ID_PECAS_LOCALIZACAO')
    SelectSQL.Strings = (
      'select * from  Pecas_Localizacao'
      'where Id_Pecas = :IdPecas'
      'and'
      'id_Concessionaria = :idConcessionaria')
    ModifySQL.Strings = (
      'update Pecas_Localizacao'
      'set'
      '  CODIGO_SGS = :CODIGO_SGS,'
      '  CORREDOR = :CORREDOR,'
      '  ESCANINHO_GAVETA = :ESCANINHO_GAVETA,'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA,'
      '  ID_PECAS = :ID_PECAS,'
      '  ID_PECAS_LOCALIZACAO = :ID_PECAS_LOCALIZACAO,'
      '  PRATELEIRA = :PRATELEIRA'
      'where'
      '  ID_PECAS_LOCALIZACAO = :OLD_ID_PECAS_LOCALIZACAO')
    ParamCheck = True
    UniDirectional = False
    Left = 488
    Top = 208
    object IntegerField4: TIntegerField
      FieldName = 'ID_PECAS_LOCALIZACAO'
      Origin = '"PECAS_LOCALIZACAO"."ID_PECAS_LOCALIZACAO"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object IntegerField5: TIntegerField
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"PECAS_LOCALIZACAO"."ID_CONCESSIONARIA"'
    end
    object IntegerField6: TIntegerField
      FieldName = 'ID_PECAS'
      Origin = '"PECAS_LOCALIZACAO"."ID_PECAS"'
    end
    object IBStringField19: TIBStringField
      FieldName = 'PRATELEIRA'
      Origin = '"PECAS_LOCALIZACAO"."PRATELEIRA"'
      Size = 3
    end
    object IBStringField20: TIBStringField
      FieldName = 'ESCANINHO_GAVETA'
      Origin = '"PECAS_LOCALIZACAO"."ESCANINHO_GAVETA"'
      Size = 2
    end
    object IBStringField21: TIBStringField
      FieldName = 'CORREDOR'
      Origin = '"PECAS_LOCALIZACAO"."CORREDOR"'
      Size = 3
    end
    object IntegerField7: TIntegerField
      FieldName = 'CODIGO_SGS'
      Origin = '"PECAS_LOCALIZACAO"."CODIGO_SGS"'
    end
  end
  object DataSourcePecasLocalizacao2: TDataSource
    AutoEdit = False
    DataSet = tblPecasLocalizacao2
    Left = 456
    Top = 208
  end
  object tblConcessionarias: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    BufferChunks = 1000
    CachedUpdates = False
    SelectSQL.Strings = (
      'Select'
      ''
      'id_concessionaria, empresa'
      ''
      'from Concessionaria')
    ParamCheck = True
    UniDirectional = False
    Left = 384
    Top = 312
    object tblConcessionariasEMPRESA: TIBStringField
      DisplayWidth = 50
      FieldName = 'EMPRESA'
      Origin = '"CONCESSIONARIA"."EMPRESA"'
      Required = True
      Size = 50
    end
    object tblConcessionariasID_CONCESSIONARIA: TIntegerField
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"CONCESSIONARIA"."ID_CONCESSIONARIA"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Visible = False
    end
  end
  object dtsConcessionarias: TDataSource
    AutoEdit = False
    DataSet = tblPecasLocalizacao
    Left = 368
    Top = 312
  end
  object dtsEstMinimo: TDataSource
    DataSet = tblEstMinimo
    Left = 368
    Top = 336
  end
  object tblEstMinimo: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    BeforePost = tblEstMinimoBeforePost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from Pecas_Estoque_Minimo'
      'where'
      '  ID_PECAS_ESTOQUE = :OLD_ID_PECAS_ESTOQUE')
    InsertSQL.Strings = (
      'insert into Pecas_Estoque_Minimo'
      '  (ID_CONCESSIONARIA, ID_PECA, ID_PECAS_ESTOQUE, QUANTIDADE)'
      'values'
      '  (:ID_CONCESSIONARIA, :ID_PECA, :ID_PECAS_ESTOQUE, :QUANTIDADE)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS_ESTOQUE,'
      '  ID_PECA,'
      '  ID_CONCESSIONARIA,'
      '  QUANTIDADE'
      'from Pecas_Estoque_Minimo '
      'where'
      '  ID_PECAS_ESTOQUE = :ID_PECAS_ESTOQUE')
    SelectSQL.Strings = (
      'Select'
      '*'
      'from Pecas_Estoque_Minimo'
      ''
      'where id_peca = :id_peca')
    ModifySQL.Strings = (
      'update Pecas_Estoque_Minimo'
      'set'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA,'
      '  ID_PECA = :ID_PECA,'
      '  ID_PECAS_ESTOQUE = :ID_PECAS_ESTOQUE,'
      '  QUANTIDADE = :QUANTIDADE'
      'where'
      '  ID_PECAS_ESTOQUE = :OLD_ID_PECAS_ESTOQUE')
    ParamCheck = True
    UniDirectional = False
    GeneratorField.Field = 'ID_PECAS_ESTOQUE'
    GeneratorField.Generator = 'GEN_PECAS_ESTOQUE_MINIMO_ID'
    Left = 384
    Top = 336
    object tblEstMinimoNOME: TStringField
      DisplayLabel = 'Concession'#225'ria'
      DisplayWidth = 66
      FieldKind = fkLookup
      FieldName = 'NOME'
      LookupDataSet = tblConcessionarias
      LookupKeyFields = 'ID_CONCESSIONARIA'
      LookupResultField = 'EMPRESA'
      KeyFields = 'ID_CONCESSIONARIA'
      Size = 49
      Lookup = True
    end
    object tblEstMinimoQUANTIDADE: TIntegerField
      DisplayLabel = 'Qtde.'
      DisplayWidth = 24
      FieldName = 'QUANTIDADE'
      Origin = '"PECAS_ESTOQUE_MINIMO"."QUANTIDADE"'
    end
    object tblEstMinimoID_PECAS_ESTOQUE: TIntegerField
      DefaultExpression = '0'
      DisplayWidth = 10
      FieldName = 'ID_PECAS_ESTOQUE'
      Origin = '"PECAS_ESTOQUE_MINIMO"."ID_PECAS_ESTOQUE"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
      Visible = False
    end
    object tblEstMinimoID_PECA: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_PECA'
      Origin = '"PECAS_ESTOQUE_MINIMO"."ID_PECA"'
      Visible = False
    end
    object tblEstMinimoID_CONCESSIONARIA: TIntegerField
      DisplayWidth = 10
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"PECAS_ESTOQUE_MINIMO"."ID_CONCESSIONARIA"'
      Visible = False
    end
  end
  object tblOrcamentoPecas: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    AfterCancel = tblPecasAfterCancel
    AfterDelete = tblPecasAfterPost
    AfterPost = tblPecasAfterPost
    BufferChunks = 1000
    CachedUpdates = False
    DeleteSQL.Strings = (
      'delete from PECAS_MOVIMENTACAO'
      'where'
      '  ID_PECAS_MOVIMENTACAO = :OLD_ID_PECAS_MOVIMENTACAO')
    InsertSQL.Strings = (
      'insert into PECAS_MOVIMENTACAO'
      
        '  (CUSTO_MEDIO, DATA, EMISSAO, ID_CONCESSIONARIA, ID_PECAS, ID_P' +
        'ECAS_MOVIMENTACAO, '
      '   MOVIMENTO, NF, QUANTIDADE_ATUAL, QUANTIDADE_MOVIMENTADA)'
      'values'
      
        '  (:CUSTO_MEDIO, :DATA, :EMISSAO, :ID_CONCESSIONARIA, :ID_PECAS,' +
        ' :ID_PECAS_MOVIMENTACAO, '
      '   :MOVIMENTO, :NF, :QUANTIDADE_ATUAL, :QUANTIDADE_MOVIMENTADA)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_PECAS_MOVIMENTACAO,'
      '  ID_CONCESSIONARIA,'
      '  ID_PECAS,'
      '  MOVIMENTO,'
      '  QUANTIDADE_MOVIMENTADA,'
      '  QUANTIDADE_ATUAL,'
      '  CUSTO_MEDIO,'
      '  DATA,'
      '  NF,'
      '  EMISSAO'
      'from PECAS_MOVIMENTACAO '
      'where'
      '  ID_PECAS_MOVIMENTACAO = :ID_PECAS_MOVIMENTACAO')
    SelectSQL.Strings = (
      'select * from ORCAMENTO_PECAS'
      'Where ID_ORCAMENTO_PECAS = :Id')
    ModifySQL.Strings = (
      'update PECAS_MOVIMENTACAO'
      'set'
      '  CUSTO_MEDIO = :CUSTO_MEDIO,'
      '  DATA = :DATA,'
      '  EMISSAO = :EMISSAO,'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA,'
      '  ID_PECAS = :ID_PECAS,'
      '  ID_PECAS_MOVIMENTACAO = :ID_PECAS_MOVIMENTACAO,'
      '  MOVIMENTO = :MOVIMENTO,'
      '  NF = :NF,'
      '  QUANTIDADE_ATUAL = :QUANTIDADE_ATUAL,'
      '  QUANTIDADE_MOVIMENTADA = :QUANTIDADE_MOVIMENTADA'
      'where'
      '  ID_PECAS_MOVIMENTACAO = :OLD_ID_PECAS_MOVIMENTACAO')
    ParamCheck = True
    UniDirectional = False
    Left = 672
    Top = 96
    object tblOrcamentoPecasID_ORCAMENTO_PECAS: TIntegerField
      FieldName = 'ID_ORCAMENTO_PECAS'
      Origin = '"ORCAMENTO_PECAS"."ID_ORCAMENTO_PECAS"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object tblOrcamentoPecasID_CLIENTES: TIntegerField
      FieldName = 'ID_CLIENTES'
      Origin = '"ORCAMENTO_PECAS"."ID_CLIENTES"'
      Required = True
    end
    object tblOrcamentoPecasID_CONCESSIONARIA: TIntegerField
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"ORCAMENTO_PECAS"."ID_CONCESSIONARIA"'
      Required = True
    end
    object tblOrcamentoPecasTOTAL_VENDA: TIBBCDField
      FieldName = 'TOTAL_VENDA'
      Origin = '"ORCAMENTO_PECAS"."TOTAL_VENDA"'
      Precision = 18
      Size = 2
    end
    object tblOrcamentoPecasTOTAL_NESTA_VENDA: TIBBCDField
      FieldName = 'TOTAL_NESTA_VENDA'
      Origin = '"ORCAMENTO_PECAS"."TOTAL_NESTA_VENDA"'
      Precision = 18
      Size = 2
    end
    object tblOrcamentoPecasSTATUS: TIBStringField
      FieldName = 'STATUS'
      Origin = '"ORCAMENTO_PECAS"."STATUS"'
    end
    object tblOrcamentoPecasDESCONTO: TIBBCDField
      FieldName = 'DESCONTO'
      Origin = '"ORCAMENTO_PECAS"."DESCONTO"'
      Precision = 18
      Size = 2
    end
    object tblOrcamentoPecasVALOR_LIQUIDO_VENDA: TIBBCDField
      FieldName = 'VALOR_LIQUIDO_VENDA'
      Origin = '"ORCAMENTO_PECAS"."VALOR_LIQUIDO_VENDA"'
      Precision = 18
      Size = 2
    end
    object tblOrcamentoPecasID_VENDEDOR: TIntegerField
      FieldName = 'ID_VENDEDOR'
      Origin = '"ORCAMENTO_PECAS"."ID_VENDEDOR"'
    end
    object tblOrcamentoPecasNUMERO_NOTA_FISCAL: TIntegerField
      FieldName = 'NUMERO_NOTA_FISCAL'
      Origin = '"ORCAMENTO_PECAS"."NUMERO_NOTA_FISCAL"'
    end
  end
  object DataSource1: TDataSource
    AutoEdit = False
    DataSet = tblOrcamentoPecas
    Left = 640
    Top = 96
  end
end
