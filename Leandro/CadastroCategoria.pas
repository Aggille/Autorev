...
type
  TBoxCategoria = class(TForm)
    ...
    tblCategoria: TSQLDataSet;
    tblCategoriaID_CATEGORIA: TIntegerField;
    tblCategoriaCODIGO: TStringField;
    tblCategoriaDESCRICAO: TStringField;
    tblCategoriaPRECO: TFMTBCDField;
    tblCategoriaPRECO_GAR: TFMTBCDField;
...
uses
   ...
   BIBLIOTECA, FDB, Pesquisa, PesquisaCategoria;
...
procedure TBoxCategoria.ClientDataSet1AfterPost(DataSet: TDataSet);
begin
...
begin
  ...
  begin
    if not Assigned(BoxPesquisarCategoria) then
//      Biblioteca.Pesquisa('Categoria', ['Id_Categoria','Descricao'],'Descricao','Id_Categoria',Fdb1.SqlConnection1);
      BoxPesquisarCategoria := TBoxPesquisarCategoria.Create(Self);
    BoxPesquisarCategoria.Show;
  end;
end;
