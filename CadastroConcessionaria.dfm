object BoxConcessionaria: TBoxConcessionaria
  Left = 5
  Top = 85
  Caption = 'Cadastro de Concession'#225'rias'
  ClientHeight = 145
  ClientWidth = 595
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesigned
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 29
    Width = 609
    Height = 115
    TabOrder = 0
    TabStop = True
    object LabelCodigo: TLabel
      Left = 14
      Top = 40
      Width = 37
      Height = 13
      Caption = 'C'#243'digo:'
    end
    object LabelDescricao: TLabel
      Left = 14
      Top = 69
      Width = 50
      Height = 13
      Caption = 'Descri'#231#227'o:'
    end
    object LabelSequencia: TLabel
      Left = 14
      Top = 13
      Width = 53
      Height = 13
      Caption = 'Sequ'#234'ncia:'
    end
    object EditSequencia: TMaskEdit
      Left = 71
      Top = 10
      Width = 48
      Height = 21
      Hint = 'Digite a sequ'#234'ncia'
      TabOrder = 0
      OnClick = EditSequenciaClick
      OnExit = EditSequenciaExit
      OnKeyPress = FormKeyPress
      OnMouseEnter = EditSequenciaClick
    end
    object JvDBMaskEditDescricao: TJvDBMaskEdit
      Left = 71
      Top = 66
      Width = 314
      Height = 21
      Hint = 'Digite a descri'#231#227'o'
      CharCase = ecUpperCase
      DataField = 'DESCRICAO'
      DataSource = DataSource1
      MaxLength = 50
      TabOrder = 2
      EditMask = 'cccccccccccccccccccccccccccccccccccccccccccccccccc;1;_'
      OnKeyPress = DBEditDescricaoKeyPress
    end
    object JvDBMaskEditCodigo: TJvDBMaskEdit
      Left = 71
      Top = 37
      Width = 68
      Height = 21
      Hint = 'Digite o c'#243'digo'
      DataField = 'CODIGO'
      DataSource = DataSource1
      MaxLength = 7
      TabOrder = 1
      EditMask = '0000.00;1;_'
      OnKeyPress = FormKeyPress
    end
  end
  object JvStatusBar1: TJvStatusBar
    Left = 0
    Top = 126
    Width = 595
    Height = 19
    Panels = <>
  end
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 595
    Height = 29
    ButtonHeight = 23
    Caption = 'ToolBar1'
    TabOrder = 2
    object BtnProcurar: TBitBtn
      Left = 0
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Procurar registro'
      Action = Procurar
      Caption = 'F4-&Procurar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        00FFFF00FFA2A2A2A3A3A39F9F9FFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnConsultar: TBitBtn
      Left = 85
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Consultar registro'
      Action = Consultar
      Caption = 'F5-C&onsultar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        B1B1B1B1B1B1B1B1B1B1B1999999FF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnNovo: TBitBtn
      Left = 170
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Registro novo'
      Action = Novo
      Caption = 'F6-&Novo'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        B4B4B4B4B4B4B4B4B4B4B4787878FF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnEditar: TBitBtn
      Left = 255
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Editar registro'
      Action = Editar
      Caption = 'F7-&Editar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        6666FF00FF666666FF00FF666666848484848484FF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnExcluir: TBitBtn
      Left = 340
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Excluir registro'
      Action = Excluir
      Caption = 'F8-E&xcluir'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnCancelar: TBitBtn
      Left = 425
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Cancelar registro'
      Action = Cancelar
      Caption = 'F9-&Cancelar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 5
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnSalvar: TBitBtn
      Left = 510
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Salvar registro'
      Action = Salvar
      Caption = 'F10-&Salvar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
  end
  object ImageList1: TImageList
    Left = 536
    Top = 40
    Bitmap = {
      
      000000000000}
  end
  object DataSource1: TDataSource
    AutoEdit = False
    DataSet = tblConcessionaria
    Left = 472
    Top = 72
  end
  object tblConcessionaria: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = IBTransaction1
    AfterCancel = tblConcessionariaAfterCancel
    AfterDelete = tblConcessionariaAfterPost
    AfterPost = tblConcessionariaAfterPost
    DeleteSQL.Strings = (
      'delete from CONCESSIONARIA'
      'where'
      '  ID_CONCESSIONARIA = :OLD_ID_CONCESSIONARIA')
    InsertSQL.Strings = (
      'insert into CONCESSIONARIA'
      
        '  (CIDADE, CNPJ, COD_CONCES, CONCESSION, EMPRESA, ENDERECO, ESTA' +
        'DO, ID_CONCESSIONARIA, '
      '   IE, TELEFONE)'
      'values'
      
        '  (:CIDADE, :CNPJ, :COD_CONCES, :CONCESSION, :EMPRESA, :ENDERECO' +
        ', :ESTADO, '
      '   :ID_CONCESSIONARIA, :IE, :TELEFONE)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_CONCESSIONARIA,'
      '  EMPRESA,'
      '  ENDERECO,'
      '  CIDADE,'
      '  ESTADO,'
      '  CNPJ,'
      '  IE,'
      '  CONCESSION,'
      '  TELEFONE,'
      '  COD_CONCES'
      'from CONCESSIONARIA '
      'where'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA')
    SelectSQL.Strings = (
      'select * from CONCESSIONARIA'
      'where Id_Concessionaria = :id')
    ModifySQL.Strings = (
      'update CONCESSIONARIA'
      'set'
      '  CIDADE = :CIDADE,'
      '  CNPJ = :CNPJ,'
      '  COD_CONCES = :COD_CONCES,'
      '  CONCESSION = :CONCESSION,'
      '  EMPRESA = :EMPRESA,'
      '  ENDERECO = :ENDERECO,'
      '  ESTADO = :ESTADO,'
      '  ID_CONCESSIONARIA = :ID_CONCESSIONARIA,'
      '  IE = :IE,'
      '  TELEFONE = :TELEFONE'
      'where'
      '  ID_CONCESSIONARIA = :OLD_ID_CONCESSIONARIA')
    Left = 504
    Top = 72
    object tblConcessionariaID_CONCESSIONARIA: TIntegerField
      FieldName = 'ID_CONCESSIONARIA'
      Origin = '"CONCESSIONARIA"."ID_CONCESSIONARIA"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object tblConcessionariaEMPRESA: TIBStringField
      FieldName = 'EMPRESA'
      Origin = '"CONCESSIONARIA"."EMPRESA"'
      Required = True
      Size = 50
    end
    object tblConcessionariaENDERECO: TIBStringField
      FieldName = 'ENDERECO'
      Origin = '"CONCESSIONARIA"."ENDERECO"'
      Required = True
      Size = 50
    end
    object tblConcessionariaCIDADE: TIBStringField
      FieldName = 'CIDADE'
      Origin = '"CONCESSIONARIA"."CIDADE"'
    end
    object tblConcessionariaESTADO: TIBStringField
      FieldName = 'ESTADO'
      Origin = '"CONCESSIONARIA"."ESTADO"'
      Required = True
      Size = 2
    end
    object tblConcessionariaCNPJ: TIBStringField
      FieldName = 'CNPJ'
      Origin = '"CONCESSIONARIA"."CNPJ"'
      Size = 18
    end
    object tblConcessionariaIE: TIBStringField
      FieldName = 'IE'
      Origin = '"CONCESSIONARIA"."IE"'
      Size = 12
    end
    object tblConcessionariaCONCESSION: TIBStringField
      FieldName = 'CONCESSION'
      Origin = '"CONCESSIONARIA"."CONCESSION"'
      Size = 15
    end
    object tblConcessionariaTELEFONE: TIBStringField
      FieldName = 'TELEFONE'
      Origin = '"CONCESSIONARIA"."TELEFONE"'
      Size = 10
    end
    object tblConcessionariaCOD_CONCES: TIBStringField
      FieldName = 'COD_CONCES'
      Origin = '"CONCESSIONARIA"."COD_CONCES"'
      Size = 9
    end
  end
  object IBTransaction1: TIBTransaction
    DefaultDatabase = FDB1.IBDatabase
    Left = 536
    Top = 72
  end
  object ActionList1: TActionList
    Images = ImageList1
    Left = 504
    Top = 40
    object Procurar: TAction
      Caption = 'F4-&Procurar'
      ShortCut = 115
      OnExecute = ProcurarExecute
    end
    object Consultar: TAction
      Caption = 'F5-C&onsultar'
      ShortCut = 116
      OnExecute = ConsultarExecute
    end
    object Novo: TAction
      Caption = 'F6-&Novo'
      ShortCut = 117
      OnExecute = NovoExecute
    end
    object Editar: TAction
      Caption = 'F7-&Editar'
      ShortCut = 118
      OnExecute = EditarExecute
    end
    object Excluir: TAction
      Caption = 'F8-E&xcluir'
      ShortCut = 119
      OnExecute = ExcluirExecute
    end
    object Cancelar: TAction
      Caption = 'F9-&Cancelar'
      ShortCut = 120
      OnExecute = CancelarExecute
    end
    object Salvar: TAction
      Caption = 'F10-&Salvar'
      ShortCut = 121
      OnExecute = SalvarExecute
    end
  end
end
