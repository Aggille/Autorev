object BoxSeguro: TBoxSeguro
  Left = 5
  Top = 85
  Caption = 'Cadastro de Seguros'
  ClientHeight = 277
  ClientWidth = 617
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poDesigned
  OnClose = FormClose
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 29
    Width = 617
    Height = 229
    Align = alClient
    TabOrder = 0
    TabStop = True
    object Label1: TLabel
      Left = 8
      Top = 51
      Width = 58
      Height = 13
      Caption = 'ID_SEGURO'
      FocusControl = SequenciaSeguro
    end
    object Label3: TLabel
      Left = 8
      Top = 88
      Width = 63
      Height = 13
      Caption = 'CORRETORA'
    end
    object Label5: TLabel
      Left = 8
      Top = 128
      Width = 27
      Height = 13
      Caption = 'Data:'
    end
    object Label7: TLabel
      Left = 8
      Top = 160
      Width = 50
      Height = 13
      Caption = 'Vendedor:'
    end
    object Label2: TLabel
      Left = 248
      Top = 51
      Width = 77
      Height = 13
      Caption = 'Pedido Ve'#237'culos:'
    end
    object Label4: TLabel
      Left = 248
      Top = 88
      Width = 63
      Height = 13
      Caption = 'Valor Pr'#234'mio:'
    end
    object Label6: TLabel
      Left = 248
      Top = 128
      Width = 37
      Height = 13
      Caption = 'Cliente:'
    end
    object SequenciaSeguro: TDBEdit
      Left = 72
      Top = 48
      Width = 59
      Height = 21
      DataField = 'ID_SEGURO'
      DataSource = Datasource1
      TabOrder = 0
    end
    object FormaPagamento: TDBLookupComboBox
      Left = 74
      Top = 85
      Width = 153
      Height = 21
      DataField = 'CORRETORA'
      DataSource = Datasource1
      KeyField = 'CORRETORA'
      ListField = 'CORRETORA'
      ListFieldIndex = 1
      ListSource = dtsCorretora
      TabOrder = 2
    end
    object Vencimento: TJvDBDateEdit
      Left = 50
      Top = 125
      Width = 115
      Height = 21
      DataField = 'DATA'
      DataSource = Datasource1
      CheckOnExit = True
      DefaultToday = True
      TabOrder = 4
    end
    object CodigoVendedor: TJvDBMaskEdit
      Left = 74
      Top = 157
      Width = 34
      Height = 21
      DataField = 'ID_VENDEDOR'
      DataSource = Datasource1
      MaxLength = 6
      TabOrder = 6
      EditMask = 'cccccc;0; '
    end
    object CodigoCliente: TJvDBMaskEdit
      Left = 331
      Top = 125
      Width = 33
      Height = 21
      DataField = 'ID_CLIENTES'
      DataSource = Datasource1
      MaxLength = 6
      TabOrder = 5
      EditMask = 'cccccc;0; '
    end
    object ValorDevido: TJvDBMaskEdit
      Left = 332
      Top = 85
      Width = 108
      Height = 21
      DataField = 'VALOR_PREMIO'
      DataSource = Datasource1
      TabOrder = 3
    end
    object Pedido: TJvDBMaskEdit
      Left = 331
      Top = 48
      Width = 34
      Height = 21
      DataField = 'ID_PEDIDO_VEICULOS'
      DataSource = Datasource1
      MaxLength = 6
      TabOrder = 1
      EditMask = 'cccccc;0; '
    end
  end
  object JvStatusBar1: TJvStatusBar
    Left = 0
    Top = 258
    Width = 617
    Height = 19
    Panels = <>
  end
  object ToolBar1: TToolBar
    Left = 0
    Top = 0
    Width = 617
    Height = 29
    ButtonHeight = 23
    Caption = 'ToolBar1'
    TabOrder = 2
    object BtnProcurar: TBitBtn
      Left = 0
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Procurar registro'
      Caption = 'F4-&Procurar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = ProcurarExecute
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        00FFFF00FFA2A2A2A3A3A39F9F9FFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnConsultar: TBitBtn
      Left = 85
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Consultar registro'
      Caption = 'F5-C&onsultar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = ConsultarExecute
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        B1B1B1B1B1B1B1B1B1B1B1999999FF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnNovo: TBitBtn
      Left = 170
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Registro novo'
      Caption = 'F6-&Novo'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
      OnClick = NovoExecute
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        B4B4B4B4B4B4B4B4B4B4B4787878FF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnEditar: TBitBtn
      Left = 255
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Editar registro'
      Caption = 'F7-&Editar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = EditarExecute
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        6666FF00FF666666FF00FF666666848484848484FF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnExcluir: TBitBtn
      Left = 340
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Excluir registro'
      Caption = 'F8-E&xcluir'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnClick = ExcluirExecute
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnCancelar: TBitBtn
      Left = 425
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Cancelar registro'
      Caption = 'F9-&Cancelar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 5
      OnClick = CancelarExecute
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
    object BtnSalvar: TBitBtn
      Left = 510
      Top = 0
      Width = 85
      Height = 23
      Hint = 'Salvar registro'
      Caption = 'F10-&Salvar'
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnClick = SalvarExecute
      OnKeyPress = FormKeyPress
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      NumGlyphs = 2
    end
  end
  object dtsCorretora: TDataSource
    AutoEdit = False
    DataSet = tblCorretora
    Left = 472
    Top = 112
  end
  object tblCorretora: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    DeleteSQL.Strings = (
      'delete from seguros'
      'where'
      '  ID_SEGURO = :OLD_ID_SEGURO')
    InsertSQL.Strings = (
      'insert into seguros'
      
        '  (CORRETORA, DATA, ID_CLIENTES, ID_PEDIDO_VEICULOS, ID_SEGURO, ' +
        'VALOR_PREMIO)'
      'values'
      
        '  (:CORRETORA, :DATA, :ID_CLIENTES, :ID_PEDIDO_VEICULOS, :ID_SEG' +
        'URO, :VALOR_PREMIO)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_SEGURO,'
      '  ID_PEDIDO_VEICULOS,'
      '  CORRETORA,'
      '  VALOR_PREMIO,'
      '  DATA,'
      '  ID_CLIENTES'
      'from seguros '
      'where'
      '  ID_SEGURO = :ID_SEGURO')
    SelectSQL.Strings = (
      'select * from seguros'
      'where ID_seguro = :Id')
    ModifySQL.Strings = (
      'update seguros'
      'set'
      '  CORRETORA = :CORRETORA,'
      '  DATA = :DATA,'
      '  ID_CLIENTES = :ID_CLIENTES,'
      '  ID_PEDIDO_VEICULOS = :ID_PEDIDO_VEICULOS,'
      '  ID_SEGURO = :ID_SEGURO,'
      '  VALOR_PREMIO = :VALOR_PREMIO'
      'where'
      '  ID_SEGURO = :OLD_ID_SEGURO')
    Left = 504
    Top = 112
    object IntegerField1: TIntegerField
      FieldName = 'ID_SEGURO'
      Origin = '"SEGUROS"."ID_SEGURO"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
      Required = True
    end
    object IntegerField2: TIntegerField
      FieldName = 'ID_PEDIDO_VEICULOS'
      Origin = '"SEGUROS"."ID_PEDIDO_VEICULOS"'
    end
    object IBStringField1: TIBStringField
      FieldName = 'CORRETORA'
      Origin = '"SEGUROS"."CORRETORA"'
      Size = 10
    end
    object IBBCDField1: TIBBCDField
      FieldName = 'VALOR_PREMIO'
      Origin = '"SEGUROS"."VALOR_PREMIO"'
      Precision = 18
      Size = 2
    end
    object DateField1: TDateField
      FieldName = 'DATA'
      Origin = '"SEGUROS"."DATA"'
    end
    object IntegerField3: TIntegerField
      FieldName = 'ID_CLIENTES'
      Origin = '"SEGUROS"."ID_CLIENTES"'
    end
  end
  object tblSeguros: TIBDataSet
    Database = FDB1.IBDatabase
    Transaction = FDB1.IBTransaction
    DeleteSQL.Strings = (
      'delete from seguros'
      'where'
      '  ID_SEGURO = :OLD_ID_SEGURO')
    InsertSQL.Strings = (
      'insert into seguros'
      
        '  (CORRETORA, DATA, ID_CLIENTES, ID_PEDIDO_VEICULOS, ID_SEGURO, ' +
        'ID_VENDEDOR, '
      '   VALOR_PREMIO)'
      'values'
      
        '  (:CORRETORA, :DATA, :ID_CLIENTES, :ID_PEDIDO_VEICULOS, :ID_SEG' +
        'URO, :ID_VENDEDOR, '
      '   :VALOR_PREMIO)')
    RefreshSQL.Strings = (
      'Select '
      '  ID_SEGURO,'
      '  ID_PEDIDO_VEICULOS,'
      '  CORRETORA,'
      '  VALOR_PREMIO,'
      '  DATA,'
      '  ID_CLIENTES,'
      '  ID_VENDEDOR'
      'from seguros '
      'where'
      '  ID_SEGURO = :ID_SEGURO')
    SelectSQL.Strings = (
      'select * from seguros'
      'where ID_seguro = :Id')
    ModifySQL.Strings = (
      'update seguros'
      'set'
      '  CORRETORA = :CORRETORA,'
      '  DATA = :DATA,'
      '  ID_CLIENTES = :ID_CLIENTES,'
      '  ID_PEDIDO_VEICULOS = :ID_PEDIDO_VEICULOS,'
      '  ID_SEGURO = :ID_SEGURO,'
      '  ID_VENDEDOR = :ID_VENDEDOR,'
      '  VALOR_PREMIO = :VALOR_PREMIO'
      'where'
      '  ID_SEGURO = :OLD_ID_SEGURO')
    Left = 504
    Top = 72
    object tblSegurosID_SEGURO: TIntegerField
      FieldName = 'ID_SEGURO'
      Origin = '"SEGUROS"."ID_SEGURO"'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object tblSegurosID_PEDIDO_VEICULOS: TIntegerField
      FieldName = 'ID_PEDIDO_VEICULOS'
      Origin = '"SEGUROS"."ID_PEDIDO_VEICULOS"'
    end
    object tblSegurosCORRETORA: TIBStringField
      FieldName = 'CORRETORA'
      Origin = '"SEGUROS"."CORRETORA"'
      Size = 10
    end
    object tblSegurosVALOR_PREMIO: TIBBCDField
      FieldName = 'VALOR_PREMIO'
      Origin = '"SEGUROS"."VALOR_PREMIO"'
      Precision = 18
      Size = 2
    end
    object tblSegurosDATA: TDateField
      FieldName = 'DATA'
      Origin = '"SEGUROS"."DATA"'
    end
    object tblSegurosID_CLIENTES: TIntegerField
      FieldName = 'ID_CLIENTES'
      Origin = '"SEGUROS"."ID_CLIENTES"'
    end
    object tblSegurosID_VENDEDOR: TIntegerField
      FieldName = 'ID_VENDEDOR'
      Origin = '"SEGUROS"."ID_VENDEDOR"'
    end
  end
  object Datasource1: TDataSource
    AutoEdit = False
    DataSet = tblSeguros
    Left = 472
    Top = 72
  end
  object ActionList1: TActionList
    Images = ImageList1
    Left = 504
    Top = 40
    object Procurar: TAction
      Caption = 'F4-&Procurar'
      ShortCut = 115
    end
    object Consultar: TAction
      Caption = 'F5-C&onsultar'
      ShortCut = 116
    end
    object Novo: TAction
      Caption = 'F6-&Novo'
      ShortCut = 117
    end
    object Editar: TAction
      Caption = 'F7-&Editar'
      ShortCut = 118
    end
    object Excluir: TAction
      Caption = 'F8-E&xcluir'
      ShortCut = 119
    end
    object Cancelar: TAction
      Caption = 'F9-&Cancelar'
      ShortCut = 120
    end
    object Salvar: TAction
      Caption = 'F10-&Salvar'
      ShortCut = 121
    end
  end
  object ImageList1: TImageList
    Left = 536
    Top = 40
    Bitmap = {
      
      000000000000}
  end
end
