unit OperacoesComprasPecasImportacao;
...
type
  TBoxComprasPecasImportacao = class(TForm)
    Panel1: TPanel;
...
var
  BoxComprasPecasImportacao: TBoxComprasPecasImportacao;
...
procedure TBoxComprasPecasImportacao.BtnImportaClick(Sender: TObject);
var
  CNPJ : String;
begin
...
procedure TBoxComprasPecasImportacao.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  ACBrNFe1.Free;
  FecharDs;
  Action :=caFree;
  BoxComprasPecasImportacao := nil;
end;

procedure TBoxComprasPecasImportacao.FormKeyPress(Sender: TObject;
  var Key: Char);
begin
  if Key = #27 then
   begin
...
procedure TBoxComprasPecasImportacao.FormShow(Sender: TObject);
begin
  BtnConfirma.Enabled := False;
end;

procedure TBoxComprasPecasImportacao.tblNofiAfterCancel(DataSet: TDataSet);
begin
  FDB1.IBtransaction.RollbackRetaining;
end;

procedure TBoxComprasPecasImportacao.tblNofiAfterPost(DataSet: TDataSet);
begin
  FDB1.IBTransaction.CommitRetaining;
end;

procedure TBoxComprasPecasImportacao.BtnImportacaoDiretaClick(Sender: TObject);
var
  CNPJ : String;
  M: TStringStream;
begin
...
procedure TBoxComprasPecasImportacao.BtnConfirmaClick(Sender: TObject);
var
//  CNPJ : String;
  i: Integer;
  total : extended;
             ...
             begin
               ...
               if recordCount = 0 then
                 begin
                   ...
                   FieldByName('Descricao').AsString := Substr(NotasFiscais.Items[0].NFe.Det.Items[i].Prod.xProd,1,60);
                   ...
                   if (FieldByName('SUBST_TRIB').AsString = 'F') then
                     if (NotasFiscais.Items[0].Nfe.Det.Items[i].Imposto.ICMS.CST = cst10) or
                      ...
                      begin
                        ...
                        //Campos para consumidor final
                        fieldByName('pRed_BCST').AsCurrency := NotasFiscais.Items[0].Nfe.Det.Items[i].Imposto.ICMS.pRedBCST;// este percentual deverá ser revisto cfe fiscalização definir
                        fieldByName('vICMS_ST').AsCurrency := NotasFiscais.Items[0].Nfe.Det.Items[i].Imposto.ICMS.vICMS;
                        fieldByName('pICMS_ST').AsCurrency := NotasFiscais.Items[0].Nfe.Det.Items[i].Imposto.ICMS.pICMS;
                        fieldByName('vBC_ST').AsCurrency := NotasFiscais.Items[0].Nfe.Det.Items[i].Imposto.ICMS.vBC;
                        FieldByName('pMVAST').AsCurrency := NotasFiscais.Items[0].Nfe.Det.Items[i].imposto.ICMS.pMVAST;
                        Post;
                      end;
                 end;
...
procedure TBoxComprasPecasImportacao.FecharDs;
begin
  tblNofi.Close;
end;
end.
